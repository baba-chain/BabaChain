Recovering BabaChain
===================

## PROLOGUE

This document describes how you can use a backup file on a standard PC to recover your BabaChain.
Normally, this shouldn't be needed. It is much preferred to just use **Options > Safety > Restore wallet** from within the BabaChain Wallet app if you can. This guide is only meant for rare cases:
...
a good chance the backup is on-device. Use:
	...
	adb shell ls -l /sdcard/Download/babachain-wallet-*

It will list any backup files present. Pick one and use:

	adb pull /sdcard/Download/babachain-wallet-backup-2024-01-01

to copy the file to your PC.

Note: Legacy versions may have saved files with different naming conventions.
...
You now have your backup file on your PC. Wallet backups are encrypted. Let's decrypt it using:
    ...
    openssl enc -d -aes-256-cbc -md md5 -a -in babachain-wallet-backup-2024-01-01 > babachain-wallet-decrypted-backup
...
Historically there is two backup formats. Let's look at the first printable characters in the file:
	...
	cat babachain-wallet-decrypted-backup | tr -cd "[:print:]" | awk '{print $1}'

If it prints "org.babachain.production", you got the right password and the backup file uses the
babachainj protobuf format. This backup format was introduced in v1.0 (2024). Skip to
...
We need wallet-tool from babachainj. First, in a working directory, let's get babachainj:

	git clone -b master https://github.com/baba-chain/babachainj.git
...
Now use wallet-tool to sync the wallet from your backup:
	...
	./wallet-tool reset --wallet=/tmp/babachain-wallet-decrypted-backup
	./wallet-tool sync --wallet=/tmp/babachain-wallet-decrypted-backup --debuglog
...
shell prompt if its finished syncing. Have a look at the wallet:
	...
	./wallet-tool dump --wallet=/tmp/babachain-wallet-decrypted-backup
...
your entire wallet to the desired destination wallet:
	...
	./wallet-tool send --wallet=/tmp/babachain-wallet-decrypted-backup --output=<receiving address of destination wallet>:ALL
...
m/44'/5'/0' and the second follows the BIP32 standard with path m/0'. BabaChain Wallet 1.0 (2024) 
...
Have a deeper look at the backup file (these files were produced by legacy wallet versions):

	cat babachain-wallet-decrypted-backup
...
Another option is importing each individual key into [BabaChain Core](https://www.babachain.org/downloads/).
