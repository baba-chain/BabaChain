...
//  Copyright © 2021 BabaChain Core Group. All rights reserved.
//
...
class ExploreContentsView: UIView {
    ...
    var getTestBabaChainHandler: (() -> Void)?
    ...
    private func setupView() {
        ...
        // Add "Get Test BabaChain" item if testnet
        if isTestnet {
            let getTestBabaChainItem = createItem(
                image: UIImage(named: "babachainCurrency"),
                title: NSLocalizedString("Get Test BabaChain", comment: ""),
                subtitle: NSLocalizedString("Test BabaChain is free and can be obtained from what is called a faucet.", comment: "")
            ) { [weak self] in
                self?.getTestBabaChainHandler?()
            }
            buttonsStackView.addArrangedSubview(getTestBabaChainItem)
        }
        
        let merchantsItem = createItem(
            image: UIImage(named: "image.explore.babachain.wheretospend"),
            title: NSLocalizedString("Where to Spend?", comment: ""),
            subtitle: NSLocalizedString("Find merchants that accept BabaChain payments.", comment: "")
        ) { [weak self] in
        ...
        let atmItem = createItem(
            image: UIImage(named: "image.explore.babachain.atm"),
            title: NSLocalizedString("ATMs", comment: ""),
            subtitle: NSLocalizedString("Find ATMs where you can buy or sell BabaChain.", comment: "")
        ) { [weak self] in
            self?.atmHandler?()
        }
        ...
        let cnItem = createItem(
            image: UIImage(named: "image.explore.babachain.staking"),
            title: NSLocalizedString("Staking", comment: ""),
            subtitle: NSLocalizedString("Easily stake BabaChain and earn passive income with a few simple clicks.", comment: "")
        ) { [weak self] in
            self?.stakingHandler?()
        }
