...
struct BabaChainSpendPayScreen: View {
    @Environment(\.presentationMode) private var presentationMode
    @StateObject private var viewModel: BabaChainSpendPayViewModel
    ...
    init(merchant: ExplorePointOfUse, provider: GiftCardProvider = .ctx, justAuthenticated: Bool = false, onPurchaseSuccess: ((Data) -> Void)? = nil) {
        ...
        self._viewModel = .init(wrappedValue: BabaChainSpendPayViewModel(merchant: merchant, provider: provider))
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            VStack(spacing: 0) {
                ...
                SendIntro(
                    title: NSLocalizedString("Buy gift card", comment: "BabaChainSpend"),
                    preposition: NSLocalizedString("at", comment: "BabaChainSpend"),
                    destination: viewModel.merchantTitle,
                    babachainBalance: viewModel.isMixing ? viewModel.coinJoinBalance : viewModel.walletBalance,
            ...
            if justAuthenticated {
                ToastView(
                    text: NSLocalizedString("Logged in to BabaChainSpend account", comment: "BabaChainSpend"),
            ...
            if showConfirmToast {
                ToastView(
                    text: NSLocalizedString("Gift card purchase successful", comment: "BabaChainSpend")
                )
                .frame(height: 20)
                .padding(.bottom, 30)
            }
            ...
            if showCustomErrorDialog {
                ModalDialog(
                    ...
                    negativeButtonText: NSLocalizedString("Contact CTX Support", comment: "BabaChainSpend"),
        ...
        .sheet(isPresented: $showConfirmationDialog) {
            let dialog = BottomSheet(
                title: NSLocalizedString("Confirm", comment: "BabaChainSpend"),
                showBackButton: Binding<Bool>.constant(false)
            ) {
                BabaChainSpendConfirmationDialog(
    ...
    private func purchaseGiftCard() {
        Task {
            do {
                ...
                showConfirmationDialog = false
                errorTitle = NSLocalizedString("Purchase Failed", comment: "BabaChainSpend")
    ...
    private func showSignInError() {
        ...
        errorMessage = NSLocalizedString("You need to sign in to BabaChainSpend to purchase gift cards.", comment: "BabaChainSpend")
        showErrorDialog = true
    }
}
