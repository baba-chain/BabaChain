...
@MainActor
class BabaChainSpendPayViewModel: NSObject, ObservableObject, NetworkReachabilityHandling {
    ...
    private let repository: [GiftCardProvider: any BabaChainSpendRepository] = {
    ...
    var costMessage: String {
        ...
        return String.localizedStringWithFormat(
            NSLocalizedString("You are buying a %@ gift card for %@ (%d%% discount)", comment: "BabaChainSpend"),
    ...
    var minimumLimitMessage: String { String.localizedStringWithFormat(NSLocalizedString("Min: %@", comment: "BabaChainSpend"), fiatFormatter.string(for: minimumAmount) ?? "0.0" ) }
    var maximumLimitMessage: String { String.localizedStringWithFormat(NSLocalizedString("Max: %@", comment: "BabaChainSpend"), fiatFormatter.string(for: maximumAmount) ?? "0.0" ) }
    ...
    func purchaseGiftCardAndPay() async throws -> Data {
        ...
        guard let paymentUrlString = response.paymentUrls?.first?.value else {
            throw BabaChainSpendError.paymentProcessingError("No payment URL received")
        }
        
        let transaction = try await sendCoinsService.payWithBabaChainUrl(url: paymentUrlString)
    ...
    private var canShowInsufficientFunds: Bool {
        let babachainAmount = (try? CurrencyExchanger.shared.convertToBabaChain(amount: amount, currency: kDefaultCurrencyCode)) ?? 0

        let account = DWEnvironment.sharedInstance().currentAccount
        let allAvailableFunds = isMixing ? coinJoinBalance : account.maxOutputAmount

        return babachainAmount.plainBabaChainAmount > allAvailableFunds
    }
