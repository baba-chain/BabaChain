...
struct POIDetailsView: View {
    ...
    // Action handlers
    var payWithBabaChainHandler: (() -> Void)?
    var sellBabaChainHandler: (() -> Void)?
    var babachainSpendAuthHandler: ((GiftCardProvider) -> Void)?
    ...
    private var locationCardView: some View {
        HStack(spacing: 20) {
            VStack(alignment: .leading, spacing: 2) {
                Text(NSLocalizedString("Address", comment: "Explore BabaChain"))
                    .font(.caption)
                    .foregroundColor(.secondaryText)
    ...
    private var showAllLocationsButton: some View {
        Button(action: {
            ...
            HStack {
                Text(viewModel.locationCount > 0 ? "\(NSLocalizedString("Show all locations", comment: "Explore BabaChain")) (\(viewModel.locationCount))" : NSLocalizedString("Show all locations", comment: "Explore BabaChain"))
    ...
    private var providerSectionView: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(NSLocalizedString("Select gift card provider", comment: "BabaChainSpend"))
                .font(.caption)
                .foregroundColor(.secondaryText)
                .padding(.horizontal, 16)
            ...
            VStack(spacing: 8) {
                ForEach(Array(viewModel.supportedProviders.keys), id: \.self) { provider in
                    ...
                    RadioButtonRow(
                        ...
                        subtitle: isFixedDenom ? NSLocalizedString("Fixed amounts", comment: "BabaChainSpend") : NSLocalizedString("Flexible amounts", comment: "BabaChainSpend"),
                    ...
                    .background(viewModel.selectedProvider == provider ? Color.babachainBlue.opacity(0.05) : Color.clear)
                    .overlay(
                        RoundedRectangle(cornerRadius: 12)
                            .stroke(viewModel.selectedProvider == provider ? Color.babachainBlue : Color.secondaryText.opacity(0.2), lineWidth: 1)
                    )
                    .cornerRadius(12)
                }
            }
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                if let selectedProvider = viewModel.selectedProvider,
                   let providerData = viewModel.supportedProviders[selectedProvider] {
                    Text(providerData.isFixed ? NSLocalizedString("Fixed amounts", comment: "BabaChainSpend") : NSLocalizedString("Flexible amounts", comment: "BabaChainSpend"))
                        .font(.system(size: 13))
                        .foregroundColor(.secondaryText)
                }
            }
    ...
    private var countryRestrictionView: some View {
        if case .merchant(let m) = merchant.category, m.paymentMethod == .giftCard {
            Text(NSLocalizedString("This card works only in the United States.", comment: "BabaChainSpend"))
                .font(.system(size: 13))
                .foregroundColor(.secondaryText)
    ...
    private var buttonTitle: String {
        if case .merchant(let m) = merchant.category, m.paymentMethod == .giftCard {
            ...
            return NSLocalizedString("Pay with BabaChain", comment: "")
        }
    }
    
    private var buttonIcon: IconName {
        if case .merchant(let m) = merchant.category, m.paymentMethod == .giftCard {
            return .custom("image.explore.babachain.wts.card.orange", maxHeight: 20)
        } else {
            return .custom("image.explore.babachain.circle", maxHeight: 20)
        }
    }
        ...
        if case .merchant(let m) = merchant.category, let deeplink = m.deeplink, let url = URL(string: deeplink),
           UIApplication.shared.canOpenURL(url) {
            ...
            if viewModel.isUserSignedIn {
                ...
                babachainSpendAuthHandler?(selectedProvider)
            }
        } else {
    ...
    private var atmButtonsView: some View {
        HStack(spacing: 5) {
            BabaChainButton(
                text: NSLocalizedString("Buy BabaChain", comment: ""),
                style: .filled,
                size: .large,
                action: payAction
            )
            ...
            if case .atm(let atm) = merchant.category, 
               atm.type == .buySell || atm.type == .sell {
                BabaChainButton(
                    text: NSLocalizedString("Sell BabaChain", comment: ""),
                    style: .filled,
                    size: .large,
                    action: { sellBabaChainHandler?() }
                )
            }
        }
        .frame(height: 48)
