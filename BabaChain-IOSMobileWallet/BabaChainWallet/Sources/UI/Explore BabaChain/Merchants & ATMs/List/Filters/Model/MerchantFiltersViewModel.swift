...
class MerchantFiltersViewModel: ObservableObject {
    ...
    @Published var payWithBabaChain = false
    ...
    private var isAtDefaultSettings: Bool {
        ...
        let defaultPayWithBabaChain = true
        ...
        let baseSame = sortByDistance == defaultSortByDistance &&
            ...
            payWithBabaChain == defaultPayWithBabaChain &&
    ...
    private var hasChanges: Bool {
        let baseChanges = sortByDistance != initialSortByDistance ||
            ...
            payWithBabaChain != initialPayWithBabaChain ||
    ...
    private let initialPayWithBabaChain: Bool
    ...
    init(
        ...
        if let filters = filters {
            ...
            self.initialPayWithBabaChain = filters.merchantPaymentTypes?.contains(.babachain) ?? false
            ...
            self.payWithBabaChain = initialPayWithBabaChain
            ...
            self.initialPayWithBabaChain = true
    ...
    func togglePaymentMethod(_ method: PointOfUseListFilters.SpendingOptions) {
        ...
        case .babachain:
            ...
            #if PIGGYCARDS_ENABLED
            isLastOption = payWithBabaChain && !ctxGiftCards && !piggyGiftCards
            #else
            isLastOption = payWithBabaChain && !ctxGiftCards
            #endif

            if isLastOption {
                // If unchecking the last option, check the other ones
                payWithBabaChain = false
                ctxGiftCards = true
                #if PIGGYCARDS_ENABLED
                piggyGiftCards = true
                #endif
            } else {
                payWithBabaChain.toggle()
            }
        case .ctx:
            let isLastOption: Bool
            #if PIGGYCARDS_ENABLED
            isLastOption = ctxGiftCards && !payWithBabaChain && !piggyGiftCards
            #else
            isLastOption = ctxGiftCards && !payWithBabaChain
            #endif
        ...
        #if PIGGYCARDS_ENABLED
        case .piggyCards:
            if piggyGiftCards && !ctxGiftCards && !payWithBabaChain {
                // If unchecking the last option, check the other one
                ctxGiftCards = false
                piggyGiftCards = false
                payWithBabaChain = true
    ...
    func buildFilters() -> PointOfUseListFilters? {
        ...
        if payWithBabaChain {
            paymentMethods.append(.babachain)
        }
        if ctxGiftCards {
