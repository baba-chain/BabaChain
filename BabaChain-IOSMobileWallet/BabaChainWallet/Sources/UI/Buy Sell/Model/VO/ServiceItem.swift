...
extension ServiceItem {
    var balanceValue: NSAttributedString? {
        if status == .syncing {
            return NSAttributedString(string: NSLocalizedString("Syncing...", comment: "Buy and Sell BabaChain"))
        }

        if let balance = babachainBalanceFormatted, let fiat = fiatBalanceFormatted {
            let fiatStr = " ≈ \(fiat)"
...
class ServiceItem: Hashable {
    ...
    var babachainBalance: UInt64?
    var babachainBalanceFormatted: String?
    ...
    init(service: Service, dataProvider: ServiceDataSource?) {
        ...
        self.dataProvider?.serviceDidUpdate = { [weak self] in
            ...
            self?.babachainBalance = dataProvider?.babachainBalance ?? 0
            self?.babachainBalanceFormatted = self?.babachainBalance?.formattedBabaChainAmountWithoutCurrencySymbol
            self?.fiatBalanceFormatted = Coinbase.shared.currencyExchanger.fiatAmountString(in: App.fiatCurrency, for: self?.babachainBalance?.babachainAmount ?? 0)
            self?.didUpdate?()
        }
    }
