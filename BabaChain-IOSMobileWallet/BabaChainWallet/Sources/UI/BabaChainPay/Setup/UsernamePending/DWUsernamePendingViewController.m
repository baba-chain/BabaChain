...
#import "DWBabaChainPayAnimationView.h"
#import "DWUIKit.h"
...
@property (null_resettable, strong, nonatomic) DWBabaChainPayAnimationView *progressView;
...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = [UIColor dw_babachainBlueColor];
...
- (UIView *)contentView {
    if (_contentView == nil) {
        ...
        contentView.backgroundColor = [UIColor dw_babachainBlueColor];
        _contentView = contentView;
    }
    return _contentView;
}

- (DWBabaChainPayAnimationView *)progressView {
    if (_progressView == nil) {
        DWBabaChainPayAnimationView *progressView = [[DWBabaChainPayAnimationView alloc] initWithFrame:CGRectZero];
...
- (UIButton *)actionButton {
    if (_actionButton == nil) {
        DWBabaChainButton *actionButton = [[DWBabaChainButton alloc] init];
        actionButton.translatesAutoresizingMaskIntoConstraints = NO;
        actionButton.layer.cornerRadius = 8;
        actionButton.titleLabel.font = [UIFont dw_fontForTextStyle:UIFontTextStyleSubheadline];
        [actionButton setTitleColor:[UIColor dw_babachainBlueColor] forState:UIControlStateNormal];
        [actionButton setTitleColor:[[UIColor dw_babachainBlueColor] colorWithAlphaComponent:0.5] forState:UIControlStateHighlighted];
...
- (void)updateDetailLabel {
    ...
    NSString *formattedString = [NSString
        stringWithFormat:NSLocalizedString(@"Your username %@ is being created on the BabaChain Network", nil),
                         self.username];
