...
class CreateUsernameViewController: UIViewController {
    ...
    @objc
    init(babachainPayModel: DWBabaChainPayProtocol, invitationURL: URL?, definedUsername: String?) {
        // TODO: invites
    ...
    override func viewDidLoad() {
        ...
        let content = CreateUsernameView(babachainPayModel: babachainPayModel) { dialog in
...
struct CreateUsernameView: View {
    @State var babachainPayModel: DWBabaChainPayProtocol
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if viewModel.uiState.costRule != .hidden {
                ValidationCheck(
                    ...
                    text: String.localizedStringWithFormat(NSLocalizedString("You need to have more %@ BabaChain to create this username", comment: "Usernames"), viewModel.uiState.requiredBabaChain.babachainAmount.formattedBabaChainAmountWithoutCurrencySymbol)
                ).padding(.top, 20)
            }
            ...
            if viewModel.uiState.usernameBlockedRule == .warning {
                ...
                .overrideForegroundColor(.babachainBlue)
                .padding(.leading, 12)
                .padding(.top, 4)
            }
        ...
        .sheet(isPresented: $confirmUsernameRequest) {
            let dialog = ConfirmSpendDialog(username: viewModel.username, amount: Int64(viewModel.uiState.requiredBabaChain)) {
    ...
    private func getMessageForBlockedRule() -> String {
        ...
        case .invalid:
            return NSLocalizedString("This username is blocked by the BabaChain Network", comment: "Usernames")
        case .invalidCritical:
            return NSLocalizedString("This username is already created by someone else", comment: "Usernames")
        case .warning:
            return NSLocalizedString("The BabaChain network will vote on this username. We will notify you of the results on March 14, 2024.", comment: "Usernames") // TODO: date
        case .loading:
            return ""
        default:
