...
final class BuyCreditsModel: SendAmountModel {
    ...
    override var isAllowedToContinue: Bool {
        return super.isAllowedToContinue && amount.babachainAmount.plainAmount / BuyCreditsModel.opCost > 0
    }
...
final class BuyCreditsViewController: SendAmountViewController, ObservableObject  {
    ...
    override func configureHierarchy() {
        ...
        let intro = SendIntro(
            ...
            babachainBalance: model.walletBalance,
            balanceLabel: NSLocalizedString("BabaChain balance", comment: "") + ":",
            avatarView: { }
        )
    ...
    override func actionButtonAction(sender: UIView) {
        ...
        checkLeftoverBalance { [weak self] canContinue in
            ...
            Task { // TODO: Temp MOCK_DASHPAY
                ...
                let babachainAmount = self.model.amount.babachainAmount.plainAmount
                let opAmount = babachainAmount / BuyCreditsModel.opCost
    ...
    private func updateRate(amount: AmountObject?) {
        ...
        let babachainAmount = max(BuyCreditsModel.opCost, amount.babachainAmount.plainAmount)
        let opAmount = babachainAmount / BuyCreditsModel.opCost
        self.rateLabel.text = String.localizedStringWithFormat(NSLocalizedString("%@ ~ %lu contacts / %lu profile updates", comment: "Credits"), babachainAmount.formattedBabaChainAmount, opAmount, opAmount)
    }
}
