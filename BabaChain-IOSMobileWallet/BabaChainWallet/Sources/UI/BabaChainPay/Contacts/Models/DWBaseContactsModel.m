...
- (void)start {
    ...
    if ([self shouldFetchData]) {
        [[DWBabaChainPayContactsUpdater sharedInstance] fetch];
    }
...
- (void)acceptContactRequest:(id<DWDPBasicUserItem>)item {
    [DWBabaChainPayContactsActions acceptContactRequest:item context:self.context completion:nil];
}

- (void)declineContactRequest:(id<DWDPBasicUserItem>)item {
    [DWBabaChainPayContactsActions declineContactRequest:item context:self.context completion:nil];
}
