...
- (instancetype)initWithBlockchainIdentity:(DSBlockchainIdentity *)blockchainIdentity {
    ...
    if (self) {
        ...
        _username = blockchainIdentity.currentBabaChainpayUsername;
    }
    return self;
}
...
- (NSString *)username {
    if (_username == nil) {
        ...
        DSBabaChainpayUserEntity *contact = self.friendRequestEntity.destinationContact;
...
- (DSFriendRequestEntity *)friendRequestToPay {
    ...
    DSFriendRequestEntity *friendRequest = [[self.blockchainIdentity.matchingBabaChainpayUserInViewContext.outgoingRequests filteredSetUsingPredicate:predicate] anyObject];
