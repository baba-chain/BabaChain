...
+ (DPAlertViewController *)insufficientFundsForInvitationAlert {
    ...
    NSString *amount = [[[priceManager stringForBabaChainAmount:DWDP_MIN_BALANCE_TO_CREATE_INVITE]
        stringByReplacingOccurrencesOfString:DASH
                                  withString:@""]
        stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];

    NSString *desc = [NSString stringWithFormat:
                                   NSLocalizedString(@"You need at least %@ BabaChain to create an invitation", nil), amount];
