...
static CGSize const BabaChainSymbolMainSize = {35.0, 27.0};
...
- (void)reloadAttributedData {
    ...
    NSString *text = NSLocalizedString(@"Each invitation will be funded with this amount so that the receiver can quickly create their username on the BabaChain Network", nil);
...
- (NSAttributedString *)mainAmountAttributedStringForAmount:(uint64_t)amount {
    return [NSAttributedString dw_babachainAttributedStringForAmount:amount
                                                      tintColor:[UIColor dw_darkTitleColor]
                                                     symbolSize:BabaChainSymbolMainSize];
}

- (NSString *)supplementaryAmountStringForAmount:(uint64_t)amount {
    DSPriceManager *priceManager = [DSPriceManager sharedInstance];
    NSString *supplementaryAmount = [priceManager localCurrencyStringForBabaChainAmount:amount];
