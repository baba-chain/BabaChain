...
@implementation DWBabaChainPayContactsActions
...
+ (void)acceptContactRequest:(id<DWDPBasicUserItem>)item
    ...
    if (MOCK_DASHPAY) {
        ...
        NSError *error = [contact applyTransientBabaChainpayUser:item.blockchainIdentity.transientBabaChainpayUser save:YES];
    ...
    void (^resultCompletion)(BOOL success, NSArray<NSError *> *errors) = ^(BOOL success, NSArray<NSError *> *errors) {
        ...
        [[DWBabaChainPayContactsUpdater sharedInstance] fetchWithCompletion:^(BOOL contactsSuccess, NSArray<NSError *> *_Nonnull contactsErrors) {
            if (completion) {
