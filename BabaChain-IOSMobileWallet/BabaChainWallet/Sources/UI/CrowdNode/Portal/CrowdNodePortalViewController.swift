...
extension CrowdNodePortalController : UITableViewDelegate, UITableViewDataSource {
    ...
    private func showMinimumBalanceError() {
        ...
        vc.headerText = NSLocalizedString("You should have a positive balance on BabaChain Wallet", comment: "CrowdNode")
        vc.descriptionText = String.localizedStringWithFormat(NSLocalizedString("Deposit at least %@ BabaChain on your BabaChain Wallet to complete a withdrawal", comment: "CrowdNode"),
                                                              CrowdNode.minimumLeftoverBalance.formattedBabaChainAmountWithoutCurrencySymbol)
        vc.actionButtonText = viewModel.buyBabaChainButtonText
...
extension CrowdNodePortalController: BalanceViewDataSource {
    var mainAmountString: String {
        viewModel.crowdNodeBalance.formattedBabaChainAmount
    }

    var supplementaryAmountString: String {
        let fiat: String

        if let fiatAmount = try? CurrencyExchanger.shared.convertBabaChain(amount: viewModel.crowdNodeBalance.babachainAmount, to: App.fiatCurrency) {
