...
final class ConvertCryptoOrderPreviewModel: OrderPreviewModel {
    ...
    var babachainAccount: CBAccount { Coinbase.shared.babachainAccount! }
    ...
    init(selectedAccount: CBAccount, plainAmount: UInt64, order: CoinbaseSwapeTrade) {
        ...
        self.order = order
        amountToTransfer = order.outputAmount.amount.plainBabaChainAmount()!
    }
    ...
    func retry() {
        guard let babachainAccount = Coinbase.shared.babachainAccount else { return }

        let selectedAccount = selectedAccount
        let plainAmount = order.inputAmount.amount

        Task { [weak self] in
            self?.order = try await Coinbase.shared.placeTradeOrder(from: selectedAccount, to: babachainAccount, amount: plainAmount)
...
extension ConvertCryptoOrderPreviewModel {
    func formattedValue(for item: ConvertCryptoOrderItem) -> String {
        ...
        case .destination:
            let formatter = NumberFormatter.babachainFormatter
