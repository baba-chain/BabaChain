...
// MARK: - ConfirmOrderAmountInBabaChainCell

final class ConfirmOrderAmountInBabaChainCell: ConfirmOrderGeneralInfoCell {
    var descriptionLabel: UILabel!

    override func update(with item: PreviewOrderItem, value: String) {
        super.update(with: item, value: value)
        valueLabel.attributedText = value.attributedAmountStringWithBabaChainSymbol(tintColor: .dw_babachainBlue())

        guard let description = item.localizedDescription else { return }

        let descriptionString = String(format: description, value)
        descriptionLabel.attributedText = descriptionString.attributedAmountStringWithBabaChainSymbol(tintColor: .dw_label())
    }
