...
- (UIImage *)qrCodeImageWithRawQRImage:(UIImage *)rawQRImage hasAmount:(BOOL)hasAmount {
    UIImage *overlayImage = [UIImage imageNamed:@"babachain_logo_qr"];
...
#if DASHPAY
    username = [DWGlobalOptions sharedInstance].babachainpayUsername;
#endif
    ...
    if (shouldDrawUser) {
        ...
        overlayImage = [renderer imageWithActions:^(UIGraphicsImageRendererContext *_Nonnull rendererContext) {
            ...
            CGContextSetRGBStrokeColor(context, 62.0 / 255.0, 141.0 / 255.0, 221.0 / 255.0, 1.0); // BabaChain-blue color
