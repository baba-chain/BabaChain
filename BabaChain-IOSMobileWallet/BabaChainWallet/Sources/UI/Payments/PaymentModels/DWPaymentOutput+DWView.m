...
- (nullable id<DWTitleDetailItem>)addressWithFont:(UIFont *)font tintColor:(UIColor *)tintColor {
    if (self.protocolRequest.commonName) {
        ...
        if (self.userItem) {
            ...
            NSAttributedString *detail = [NSAttributedString dw_babachainAddressAttributedString:address withFont:font];
            DWTitleDetailCellModel *model =
...
- (nullable id<DWTitleDetailItem>)feeWithFont:(UIFont *)font tintColor:(UIColor *)tintColor {
    ...
    NSAttributedString *feeString = [NSAttributedString dw_babachainAttributedStringForAmount:feeValue
                                                                               tintColor:tintColor
                                                                                    font:font];
...
- (id<DWTitleDetailItem>)totalWithFont:(UIFont *)font tintColor:(UIColor *)tintColor {
    NSAttributedString *detail = [NSAttributedString dw_babachainAttributedStringForAmount:self.amount
                                                                            tintColor:tintColor
                                                                                 font:font];
...
- (nullable NSString *)generalInfoString {
    ...
    if (self.localCurrency && ![self.localCurrency isEqualToString:DWApp.localCurrencyCode]) {
        NSString *requestedAmount = [CurrencyExchangerObjcWrapper fiatCurrencyString:self.localCurrency forBabaChainAmount:self.amount];
        if (info.length > 0) {
