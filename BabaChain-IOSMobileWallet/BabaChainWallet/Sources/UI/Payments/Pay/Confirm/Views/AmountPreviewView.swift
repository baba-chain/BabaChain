...
private let kBabaChainSymbolMainSize = CGSize(width: 35.0, height: 27.0)
...
final class AmountPreviewView: UIView {
    ...
    private func mainAmountAttributedString(forAmount amount: UInt64) -> NSAttributedString {
        NSAttributedString.babachainAttributedString(for: amount,
                                                tintColor: UIColor.dw_darkTitle(),
                                                symbolSize: kBabaChainSymbolMainSize)
    }

    private func supplementaryAmountString(forAmount amount: UInt64) -> String {
        let supplementaryAmount = CurrencyExchanger.shared.fiatAmountString(for: amount.babachainAmount)
