...
- (void)refreshOptions {
    ...
    if (blockchainIdentity.currentBabaChainpayUsername != nil || MOCK_DASHPAY) {
        DWPayOptionModel *option = [[DWPayOptionModel alloc] initWithType:DWPayOptionModelType_BabaChainPayUser];
...
- (void)performNFCReadingWithCompletion:(void (^)(DWPaymentInput *paymentInput))completion {
    ...
    dispatch_queue_t queue = dispatch_queue_create("org.babachain.nfc-reading-queue", DISPATCH_QUEUE_CONCURRENT);
...
- (void)readerSession:(nonnull NFCNDEFReaderSession *)session didInvalidateWithError:(nonnull NSError *)error {
    ...
    if (FALSE) {
        ...
        __unused NSString *s = NSLocalizedString(@"NFC device didn't transmit a valid BabaChain address", nil);
    }
#endif
