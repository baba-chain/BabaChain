...
extension UIButton.Configuration {
    ...
    public static func babachainPlain() -> UIButton.Configuration {
        var configuration = configuration(from: .plain())
        configuration.baseForegroundColor = .dw_babachainBlue()
        return configuration
    }

    public static func babachainGray() -> UIButton.Configuration {
    ...
    public static var tinted: UIButton.Configuration {
        ...
        configuration.baseBackgroundColor = .dw_babachainBlue().withAlphaComponent(0.08)
        configuration.baseForegroundColor = .dw_babachainBlue()

        var background = configuration.background
        background.backgroundColor = UIColor.dw_babachainBlue().withAlphaComponent(0.08)
    ...
    public static func action() -> UIButton.Configuration {
        ...
        configuration.baseBackgroundColor = .dw_babachainBlue()
        configuration.background = background
        return configuration
    }
...
class TintedButton: ActivityIndicatorButton {
    ...
    override func updateConfiguration() {
        ...
        let accentColor: UIColor = (accentColor ?? configuration.baseForegroundColor) ?? .dw_babachainBlue()
...
final class GrayButton: ActivityIndicatorButton {
    init() {
        super.init(configuration: .babachainGray())
    }

    required init?(coder: NSCoder) {
        super.init(coder: coder)

        configuration = .babachainGray()
    }
...
final class PlainButton: ActivityIndicatorButton {
    init() {
        super.init(configuration: .babachainPlain())
    }

    required init?(coder: NSCoder) {
        super.init(coder: coder)

        configuration = .babachainPlain()
    }
    ...
    override func updateConfiguration() {
        ...
        let accentColor = (accentColor ?? configuration.baseForegroundColor) ?? .dw_babachainBlue()
...
class ActivityIndicatorButton: DWBabaChainButton {
...
// MARK: - BabaChainButton

class DWBabaChainButton: UIButton {
    @IBInspectable
    ...
    convenience init() {
        self.init(configuration: .babachainPlain())
    }

    init(configuration: UIButton.Configuration = UIButton.Configuration.babachainPlain()) {
        super.init(frame: .zero)
