...
struct ButtonsGroup: View {
    ...
    var size: BabaChainButton.Size = .medium
    ...
    var positiveButton: BabaChainButton? {
        if let text = positiveButtonText {
            return BabaChainButton(
                text: text,
    ...
    var negativeButton: BabaChainButton? {
    ...
    var body: some View {
        if orientation == .vertical {
            VStack(spacing: 6) {
                positiveButton
                    .overrideBackgroundColor(style == .regular ? .babachainBlue : .buttonRed)
                    .overrideForegroundColor(negativeButtonText == nil ? .babachainBlue : .white)
                negativeButton
                    .overrideForegroundColor(positiveButtonText == nil ? .babachainBlue : .primaryText)
            }
        } else {
            HStack(spacing: 6) {
                negativeButton
