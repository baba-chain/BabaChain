    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            if let text = actionText, let action = action {
                BabaChainButton(text: text, style: .plain, size: .extraSmall, stretch: false, action: action)
                    .overrideForegroundColor(Color.primaryBackground)
            }
            
            if let icon = closeButtonIcon, let action = closeAction {
                BabaChainButton(leadingIcon: icon, style: .plain, size: .small, stretch: false, action: action)
                    .overrideForegroundColor(Color.primaryBackground)
            }
        }
        .padding(.horizontal, 8)
