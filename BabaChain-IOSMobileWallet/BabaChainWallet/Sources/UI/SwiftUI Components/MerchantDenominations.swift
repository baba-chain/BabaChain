...
struct MerchantDenominations: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            Text(NSLocalizedString("Select amount", comment: "BabaChainSpend"))
                .font(.h5Bold)
                .foregroundColor(.primaryText)
            
            Text(NSLocalizedString("This merchant sells gift cards at fixed prices", comment: "BabaChainSpend"))
                .font(.body2)
            ...
            Text(formattedValue)
                ...
                .background(
                    RoundedRectangle(cornerRadius: 14)
                        .fill(isSelected ? Color.babachainBlue.opacity(0.1) : Color.clear)
                )
                .overlay(
                    RoundedRectangle(cornerRadius: 14)
                        .stroke(
                            isSelected ? Color.babachainBlue : Color.primaryText.opacity(0.08),
                            lineWidth: 1.5
                        )
                )
        }
