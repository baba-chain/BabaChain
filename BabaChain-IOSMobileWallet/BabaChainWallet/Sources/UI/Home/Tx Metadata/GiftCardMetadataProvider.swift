...
class GiftCardMetadataProvider: MetadataProvider, @unchecked Sendable {
    ...
    private func loadMetadata() async {
        ...
        for giftCard in giftCards {
            let title = String.localizedStringWithFormat(NSLocalizedString("Gift card · %@", comment: "BabaChainSpend"), giftCard.merchantName)
            ...
            metadataQueue.async { [weak self] in
                ...
                if txRowMetadata != nil {
                    ...
                    txRowMetadata!.secondaryIcon = .custom("image.explore.babachain.wts.payment.gift-card")
                } else {
                    txRowMetadata = TxRowMetadata(
                        title: title,
                        secondaryIcon: .custom("image.explore.babachain.wts.payment.gift-card")
                    )
                }
