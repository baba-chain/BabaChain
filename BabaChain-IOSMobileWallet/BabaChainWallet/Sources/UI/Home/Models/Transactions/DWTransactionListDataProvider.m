...
- (id<DWTransactionListDataItem>)transactionDataForTransaction:(DSTransaction *)transaction {
    ...
    switch (transactionDirection) {
        case DSTransactionDirection_Moved: {
            dataItem.babachainAmount = [account amountReceivedFromTransactionOnExternalAddresses:transaction];
            dataItem.outputReceiveAddresses = [account externalAddressesOfTransaction:transaction];
            break;
        }
        case DSTransactionDirection_Sent: {
            dataItem.babachainAmount = [chain amountSentByTransaction:transaction] - [chain amountReceivedFromTransaction:transaction] - transaction.feeUsed;
            dataItem.outputReceiveAddresses = [account externalAddressesOfTransaction:transaction];
            break;
        }
        case DSTransactionDirection_Received: {
            dataItem.babachainAmount = [account amountReceivedFromTransaction:transaction];
            dataItem.outputReceiveAddresses = [account externalAddressesOfTransaction:transaction];
            break;
        }
        case DSTransactionDirection_NotAccountFunds: {
            dataItem.babachainAmount = 0;
