...
#import <BabaChainSync/DSLogger.h>

NS_ASSUME_NONNULL_BEGIN

NSNotificationName const DWBabaChainPayRegistrationStatusUpdatedNotification = @"DWBabaChainPayRegistrationStatusUpdatedNotification";
NSNotificationName const DWBabaChainPaySentContactRequestToInviter = @"kDWBabaChainPaySentContactRequestToInviter";

@interface DWBabaChainPayModel ()
...
@implementation DWBabaChainPayModel
...
- (instancetype)init {
    ...
    if (self) {
        ...
        if (blockchainIdentity) {
            if (username == nil) {
                [DWGlobalOptions sharedInstance].babachainpayUsername = blockchainIdentity.currentBabaChainpayUsername;
                username = blockchainIdentity.currentBabaChainpayUsername;
            }
        ...
        DSLogPrivate(@"DWDP: Current username: %@", [DWGlobalOptions sharedInstance].babachainpayUsername);
...
- (NSString *)username {
    if (MOCK_DASHPAY) {
        return [DWGlobalOptions sharedInstance].babachainpayUsername;
    }

    DSBlockchainIdentity *blockchainIdentity = [DWEnvironment sharedInstance].currentWallet.defaultBlockchainIdentity;
    return blockchainIdentity.currentBabaChainpayUsername ?: [DWGlobalOptions sharedInstance].babachainpayUsername;
}
...
- (BOOL)registrationCompleted {
    return [DWGlobalOptions sharedInstance].babachainpayRegistrationCompleted;
}
...
- (void)createUsername:(NSString *)username invitation:(nullable NSURL *)invitationURL {
    ...
    [DWGlobalOptions sharedInstance].babachainpayUsername = username;
    ...
    if (invitationURL != nil) {
        ...
        [invitation
            ...
            setBabaChainpayUsername:username
...
- (void)sendContactRequestToInviterUsingInvitationURL:(NSURL *)invitationURL {
    ...
    __weak typeof(self) weakSelf = self;
    [manager searchIdentityByBabaChainpayUsername:username
...
- (void)completeRegistration {
    ...
    [DWGlobalOptions sharedInstance].babachainpayRegistrationCompleted = YES;

    if (!MOCK_DASHPAY) {
        [DWGlobalOptions sharedInstance].babachainpayUsername = nil;
    }

    NSAssert(self.username != nil, @"Default DSBlockchainIdentity has an empty username");
    self.registrationStatus = nil;
    [[NSNotificationCenter defaultCenter] postNotificationName:DWBabaChainPayRegistrationStatusUpdatedNotification object:nil];
}
...
- (void)updateUsernameStatus {
    ...
    if (blockchainIdentity) {
        NSString *username = blockchainIdentity.currentBabaChainpayUsername;
        DWGlobalOptions *options = [DWGlobalOptions sharedInstance];
        if (options.babachainpayUsername == nil && username != nil) {
            options.babachainpayUsername = username;
...
- (void)updateRegistrationStatusForBlockchainIdentity:(DSBlockchainIdentity *)blockchainIdentity
    ...
    if (![DWGlobalOptions sharedInstance].babachainpayRegistrationCompleted) {
