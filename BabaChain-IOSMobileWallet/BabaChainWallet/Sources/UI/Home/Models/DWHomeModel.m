...
#import "UIDevice+BabaChainWallet.h"
...
@property (nonatomic, strong) id<DWBabaChainPayProtocol> babachainPayModel;
...
@synthesize babachainPayModel = _babachainPayModel;
...
#if DASHPAY
        _babachainPayModel = [[DWBabaChainPayModel alloc] init];
...
- (BOOL)performOnSetupUpgrades {
    ...
    DSVersionManager *babachainSyncVersionManager = [DSVersionManager sharedInstance];
    NSArray *wallets = [DWEnvironment sharedInstance].allWallets;

    [babachainSyncVersionManager
        upgradeExtendedKeysForWallets:wallets
                       ...
                       withCompletion:^(BOOL success, BOOL neededUpgrade, BOOL authenticated, BOOL cancelled) {
                           DWVersionManager *babachainwalletVersionManager = [DWVersionManager sharedInstance];
                           [babachainwalletVersionManager
...
- (void)walletDidWipe {
#if DASHPAY
    self.babachainPayModel = [[DWBabaChainPayModel alloc] init];
#endif /* DASHPAY_ENABLED */
}
...
#pragma mark - DWBabaChainPayReadyProtocol
...
- (BOOL)shouldShowCreateUserNameButton {
    ...
    if (self.babachainPayModel.registrationStatus != nil) {
        return NO;
    }

    if (self.babachainPayModel.registrationCompleted) {
        return NO;
    }
...
- (void)handleDeeplink:(NSURL *)url
    ...
    [self.babachainPayModel verifyDeeplink:url completion:completion];
}
#endif
...
#if DASHPAY
    [[DWBabaChainPayContactsUpdater sharedInstance] endUpdating];
#endif
}
...
- (void)syncingActivityMonitorStateDidChangeWithPreviousState:(enum SyncingActivityMonitorState)previousState state:(enum SyncingActivityMonitorState)state {
    ...
    if (isSynced) {
        [self.babachainPayModel updateUsernameStatus];

        if (self.babachainPayModel.username != nil) {
            [self.receiveModel updateReceivingInfo];
#if DASHPAY
            [[DWBabaChainPayContactsUpdater sharedInstance] beginUpdating];
#endif
        }
