...
class Transaction: TransactionDataItem, Identifiable {
    ...
    private lazy var _babachainAmount: UInt64 = tx.babachainAmount
    
    var babachainAmount: UInt64 { _babachainAmount }
    var signedBabaChainAmount: Int64 {
        if babachainAmount == UInt64.max {
            return Int64.max
        }
        
        return direction == .sent ? -Int64(babachainAmount) : Int64(babachainAmount)
    }
    ...
    private lazy var storedFiatAmount = userInfo?.fiatAmountString(from: _babachainAmount) ?? NSLocalizedString("Not available", comment: "");
    ...
    var stateTitle: String {
        ...
        case .blockchainIdentityRegistration:
            return NSLocalizedString("BabaChainPay Upgrade Fee", comment: "")
        }
    }
...
extension Transaction {
    ...
    var babachainAmountTintColor: UIColor {
        direction.babachainAmountTintColor
    }
...
extension Transaction: Hashable {
    ...
    func hash(into hasher: inout Hasher) {
        hasher.combine(babachainAmount)
