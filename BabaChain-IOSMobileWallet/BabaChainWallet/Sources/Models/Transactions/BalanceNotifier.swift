...
class DWBalanceNotifier: NSObject {
    ...
    func registerForPushNotifications() {
        NotificationCenter.default.post(name: NSNotification.Name("org.babachain.will-request-permission-notification"), object: nil)
        let options: UNAuthorizationOptions = [.badge, .sound, .alert]
        UNUserNotificationCenter.current().requestAuthorization(options: options) { granted, error in
            DispatchQueue.main.async {
                NotificationCenter.default.post(name: NSNotification.Name("org.babachain.did-request-permission-notification"), object: nil)
            }
    ...
    private func walletBalanceDidChangeNotification(note: Notification) {
        ...
        if balance < wallet.balance {
            ...
            if isCrowdNode {
                ...
                let receivedAmountText = received.formattedBabaChainAmount
                let receivedInFiatText = CurrencyExchanger.shared.fiatAmountString(for: received.babachainAmount)
