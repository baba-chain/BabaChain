...
class MerchantDAO: PointOfUseDAO {
    ...
    let serialQueue = DispatchQueue(label: "org.babachainfoundation.babachainpaytnt.explore.serial.queue")
    ...
    func items(query: String?,
        ...
        serialQueue.async { [weak self] in
            ...
            if let methods = paymentMethods {
                ...
                if methods.contains(PointOfUseListFilters.SpendingOptions.babachain) {
                    tempMethods.append(ExplorePointOfUse.Merchant.PaymentMethod.babachain)
                }
                ...
                if !methods.contains(PointOfUseListFilters.SpendingOptions.babachain) && (hasCTX != hasPiggy) {
            ...
            if let denominationType = denominationType {
                ...
                case .fixed:
                    // Include all babachain merchants OR gift card merchants with "fixed" denomination
                    queryFilter = queryFilter && (paymentMethodColumn == "babachain" || Expression<Bool>(literal: "denominationsType = 'fixed'"))
                case .flexible:
                    // Include all babachain merchants OR gift card merchants with "min-max" denomination
                    queryFilter = queryFilter && (paymentMethodColumn == "babachain" || Expression<Bool>(literal: "denominationsType = 'min-max'"))
                case .both:
