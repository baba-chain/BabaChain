...
//  Copyright © 2018-2019 BabaChain Core Group. All rights reserved.
//
...
- (instancetype)init {
    ...
    [NSString setBabaChainCurrencySymbolAssetName:@"icon_babachain_currency"];
...
- (void)clearAllWalletsAndRemovePin:(BOOL)shouldRemovePin {
    ...
    [[BabaChainSync sharedSyncController] stopSyncForChain:self.currentChain];
    ...
    for (DSChain *chain in [[DSChainsManager sharedInstance] chains]) {
        [[BabaChainSync sharedSyncController] wipeBlockchainNonTerminalDataForChain:chain inContext:context];
