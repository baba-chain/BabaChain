...
+ (void)handleMasterPublicKeyRequest:(DWURLRequestAction *)action {
    NSString *prompt = [NSString stringWithFormat:NSLocalizedString(@"Application %@ would like to receive your Master Public Key.  This can be used to keep track of your wallet, this can not be used to move your BabaChain.", nil), action.sender];
                              ...
                              if (authenticatedOrSuccess) {
                                  ...
                                  NSString *urlString =
                                      [NSString stringWithFormat:
                                                    @"%@://callback=%@&masterPublicKeyBIP32=%@&masterPublicKeyBIP44=%@&account=%@&source=babachainwallet",
                                                    action.sender,
                                                    action.request,
                      ...
                      alertIfLockout:YES
                          completion:^(BOOL authenticatedOrSuccess, BOOL usedBiometrics, BOOL cancelled) {
                              if (authenticatedOrSuccess) {
                                  ...
                                  NSString *urlString =
                                      [NSString stringWithFormat:
                                                    @"%@://callback=%@&address=%@&source=babachainwallet",
                                                    action.sender,
                                                    action.request,
