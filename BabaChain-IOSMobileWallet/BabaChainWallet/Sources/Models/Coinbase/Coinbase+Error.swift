    ...
    enum Error: Swift.Error, LocalizedError {
        enum GeneralFailureReason: LocalizedError {
            ...
            var errorDescription: String? {
                ...
                case .noPaymentMethods:
                    return NSLocalizedString("Please add a payment method on Coinbase", comment: "Coinbase/Buy BabaChain")
                case .noCashAccount:
            ...
            var failureReason: String? {
                switch self {
                case .noPaymentMethods:
                    return NSLocalizedString("No payment methods found", comment: "Coinbase/Buy BabaChain")
                default:
                    return ""
                }
            }
            ...
            var recoverySuggestion: String? {
                switch self {
                case .noPaymentMethods:
                    return NSLocalizedString("Add", comment: "Coinbase/Buy BabaChain")
                default:
                    return ""
                }
            }
        ...
        enum TransactionFailureReason: LocalizedError, Identifiable, Equatable {
            ...
            var errorDescription: String? {
                switch self {
                case .invalidAmount:
                    let amountString = DSTransaction.txMinOutputAmount().formattedBabaChainAmount
                    return String(format: NSLocalizedString("BabaChain payments can't be less than %@", comment: "Coinbase"), amountString)
                case .failedToObtainNewAddress:
