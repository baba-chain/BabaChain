...
struct CoinbaseUserAccountData: Codable, Identifiable {
    ...
    var plainAmount: UInt64 {
        guard let babachainNumber = Decimal(string: balance.amount) else {
            return 0
        }

        let plainAmount = babachainNumber * pow(10, currency.exponent)
        return NSDecimalNumber(decimal: plainAmount).uint64Value
    }

    var plainAmountInBabaChain: UInt64 {
        if currencyCode == kBabaChainCurrency { return plainAmount }

        guard let babachainAmount = try? Coinbase.shared.currencyExchanger.convertToBabaChain(amount: balance.amount.decimal()!, currency: currencyCode) else {
            return 0
        }

        return babachainAmount.plainBabaChainAmount
    }
