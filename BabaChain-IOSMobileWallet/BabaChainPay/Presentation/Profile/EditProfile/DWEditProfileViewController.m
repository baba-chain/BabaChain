...
- (BOOL)hasChanges {
    ...
    if (self.aboutMe && ![self.aboutMe isEqualToString:self.blockchainIdentity.matchingBabaChainpayUserInViewContext.publicMessage]) {
        return YES;
    }
...
- (void)setupItems {
...

    {
        ...
        cellModel.text = self.blockchainIdentity.matchingBabaChainpayUserInViewContext.publicMessage;
        __weak typeof(self) weakSelf = self;
        cellModel.didChangeValueBlock = ^(DWTextFieldFormCellModel *_Nonnull cellModel) {
            __strong typeof(weakSelf) strongSelf = weakSelf;
            if (!strongSelf) {
