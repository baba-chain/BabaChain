...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Send 210 BABACHAIN from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 1000 BABACHAIN in block rewards plus fees, but
        ...
        # Send 100 BABACHAIN with subtract fee from amount
        ...
        # Sendmany 100 BABACHAIN with subtract fee from amount
        ...
        if not self.options.descriptors:
            ...
            # This will raise an exception for attempting to get the private key of an invalid BabaChain address
            assert_raises_rpc_error(-5, "Invalid BabaChain address", self.nodes[0].dumpprivkey, "invalid")

            # This will raise an exception for attempting to set a label for an invalid BabaChain address
            assert_raises_rpc_error(-5, "Invalid BabaChain address", self.nodes[0].setlabel, "invalid address", "label")

            # This will raise an exception for importing an invalid address
            assert_raises_rpc_error(-5, "Invalid BabaChain address or script", self.nodes[0].importaddress, "invalid")
            ...
            for target, mode in product([-1, 0], ["babachain/kb", "duff/b"]):
                assert_raises_rpc_error(-8, 'Invalid estimate_mode parameter, must be one of: "unset", "economical", "conservative"',
                    self.nodes[2].sendtoaddress, address=address, amount=1, conf_target=target, estimate_mode=mode)
