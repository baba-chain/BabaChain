...
TMPDIR_PREFIX = "babachain_func_test_"
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave babachainds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop babachainds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use babachain-cli instead of RPC for all commands")
        parser.add_argument("--babachaind-arg", dest="babachaind_extra_args", default=[], action="append",
                            help="Pass extra args to all babachaind instances")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "babachaind": ("bitcoind", "BABACHAIND"),
            "babachain-cli": ("bitcoincli", "BABACHAINCLI"),
            "babachain-util": ("bitcoinutil", "BABACHAINUTIL"),
            "babachain-wallet": ("bitcoinwallet", "BABACHAINWALLET"),
        }
    ...
    def setup(self):
        ...
        self.extra_args_from_options = self.options.babachaind_extra_args
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: babachainds were not stopped and may still be running")
    ...
    def setup_nodes(self):
        ...
        """ NOTE! If this method is updated - backport changes to  BabaChainTestFramework.setup_nodes"""
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'babachaind', self.options.bitcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'babachain-cli', self.options.bitcoincli) for v in versions]
    ...
    # TODO: move it to BabaChainTestFramework where it belongs
    def dynamically_initialize_datadir(self, node_p2p_port, node_rpc_port):
        ...
        with open(os.path.join(new_data_dir, "babachain.conf"), 'w', encoding='utf8') as f:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as babachaind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in babachain.conf
    ...
    def skip_if_no_bitcoind_tracepoints(self):
        """Skip the running test if babachaind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("babachaind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if babachaind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("babachaind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if babachain-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("babachain-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if babachain-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("babachain-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether babachain-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether babachain-wallet was compiled."""
...
class BabaChainTestFramework(BitcoinTestFramework):
    def set_test_params(self):
    ...
    def set_babachain_test_params(self, num_nodes, masterodes_count, extra_args=None, evo_count=0):
        ...
        # it can't be disabled for BabaChainTestFramework same as BitcoinTestFramework
        ...
        # This is nRequestTimeout in babachain-q-recovery thread
    ...
    def set_babachain_llmq_test_params(self, llmq_size, llmq_threshold):
