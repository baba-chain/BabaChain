...
"""Class for babachaind node under test"""
...
class TestNode():
    """A class for representing a babachaind node under test.
    ...
    def __init__(self, i, datadir, extra_args_from_options, *, chain, rpchost, timewait, timeout_factor, bitcoind, bitcoin_cli, mocktime, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False, v2transport=False):
        ...
        self.chain = chain
        self.bitcoinconf = os.path.join(self.datadir, "babachain.conf")
    ...
    def __del__(self):
        # Ensure that we don't leave any babachaind processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time babachaind is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by babachaind, and
        ...
        self.running = True
        self.log.debug("babachaind started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the babachaind process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'babachaind exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; babachaind is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to babachaind after {}s".format(self.rpc_timeout))
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to babachaind
        expected_msg: regex that stderr should match when babachaind fails

        Will throw if babachaind starts without an error.
        Will throw if an expected_msg is provided and it does not match babachaind's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'babachaind exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'babachaind should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to babachain-cli for an individual node"""
    def __init__(self, binary, datadir):
        self.options = []
        self.binary = binary
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.babachaincli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with babachain-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run babachain-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running babachain-cli {}".format(p_args[2:]))
