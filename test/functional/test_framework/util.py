...
def assert_fee_amount(fee, tx_size, feerate_BABACHAIN_kvB):
    """Assert the fee is in range."""
    feerate_BABACHAIN_vB = feerate_BABACHAIN_kvB / 1000
    target_fee = satoshi_round(tx_size * feerate_BABACHAIN_vB)
    if fee < target_fee:
        raise AssertionError("Fee of %s BABACHAIN too low! (Should be %s BABACHAIN)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * feerate_BABACHAIN_vB:
        raise AssertionError("Fee of %s BABACHAIN too high! (Should be %s BABACHAIN)" % (str(fee), str(target_fee)))
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "babachain.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "babachain.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "babachain.conf")):
        with open(os.path.join(datadir, "babachain.conf"), 'r', encoding='utf8') as f:
            for line in f:
