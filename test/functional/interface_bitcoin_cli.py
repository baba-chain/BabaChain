...
# The block reward of coinbaseoutput.nValue (500) BABACHAIN/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 500 BABACHAIN/block.
...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `babachain-cli getblockchaininfo`")
        ...
        self.log.info("Test -getinfo and babachain-cli return all proxies")
        ...
        if self.is_specified_wallet_compiled():
            self.log.info("Test -getinfo and babachain-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (BABACHAIN/kB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (BABACHAIN/kB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (500 BABACHAIN) to the default wallet.
