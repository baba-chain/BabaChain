...
The data directory is the default location where the BabaChain Core files are stored.

1. The default data directory paths for supported platforms are:

Platform | Data directory path
---------|--------------------
Linux    | `$HOME/.babachaincore/`
macOS    | `$HOME/Library/Application Support/BabaChaincore/`
Windows  | `%APPDATA%\BabaChaincore\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `babachain.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual BabaChain blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `babachain.conf`        | User-defined [configuration settings](babachain-conf.md) for `babachaind` or `babachain-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `babachaind.pid`        | Stores the process ID (PID) of `babachaind` or `babachain-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `babachaind` or `babachain-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [babachain.conf](babachain-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`babachain-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by BabaChain Core:
...
`blk000?.dat`  | Block data (custom format, 2 GiB per file); replaced by `blocks/blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> in 0.8.0 | [PR #1677](https://github.com/babachain/babachain/pull/1677)
`addr.dat`     | Peer IP address BDB database; replaced by `peers.dat` in [0.7.0](https://github.com/babachain/babachain/blob/master/doc/release-notes/release-notes-0.7.0.md) | [PR #1198](https://github.com/babachain/babachain/pull/1198), [`928d3a01`](https://github.com/babachain/babachain/commit/928d3a011cc66c7f907c4d053f674ea77dc611cc)
