...
## Summary
The compact block filter index format has been updated to include BabaChain special transaction data, providing feature parity with bloom filters for SPV client support. This change is **incompatible** with existing blockfilter indexes. Existing blockfilter indexes will automatically be re-created with the new version.

## Technical Details
- The blockfilter index now includes fields from BabaChain special transactions:
...
## Benefits
- SPV clients can now detect and track BabaChain-specific transactions
