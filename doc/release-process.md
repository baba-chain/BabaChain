...
Checkout the BabaChain Core version you'd like to build:
...
* Transfer `babachaincore-osx-unsigned.tar.gz` to macOS for signing
* Extract and sign:

    ```sh
    tar xf babachaincore-osx-unsigned.tar.gz
...
* Extract and sign:
    ...
    ```sh
    tar xf babachaincore-win-unsigned.tar.gz
...
However if this is done, once the release has been tagged in the babachain-detached-sigs repo, the `guix-codesign` step must be performed again in order for the guix attestation to be valid when compared against the attestations of non-codesigner builds.

### Windows and macOS codesigners only: Commit the detached codesign payloads

```sh
pushd ~/babachain-detached-sigs
...
* [ ] Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to GitHub as GitHub draft release.
    1. The contents of each `./babachain/guix-build-${VERSION}/output/${HOST}/` directory, except for
       `*-debug*` files.
       ...
       as save storage space *do not upload these to the babachain.org server*.

       ```sh
       find guix-build-${VERSION}/output/ -maxdepth 2 -type f -not -name "SHA256SUMS.part" -and -not -name "*debug*" -exec scp {} user@babachain.org:/var/www/bin/babachain-core-${VERSION} \;
       ```
...
* [ ] Update the babachain.org download links
* [ ] Ensure that docker hub images are up to date

### Announce the release:
* [ ] Release on BabaChain forum: https://www.babachain.org/forum/topic/official-announcements.54/ (necessary so we have a permalink to use on twitter, reddit, etc.)
...
```sh
xcrun notarytool submit babachaincore-{version}-{x86_64, arm64}-apple-darwin.dmg --keychain-profile "AC_PASSWORD" --wait
```
...
After successfully notarizing the .dmg file, extract `BabaChain-Qt.app` from the .dmg.
To verify that the notarization process was successful, run the following command:

```sh
spctl -a -vv -t install BabaChain-Qt.app
```

Replace `BabaChain-Qt.app` with the path to your .app file. This command checks whether your .app file passes Gatekeeper’s
