# CJDNS support in BabaChain Core

It is possible to run BabaChain Core over CJDNS, an encrypted IPv6 network that
...
done outside of BabaChain Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the BabaChain network and individual nodes.
...
before it will work with your BabaChain Core node. This is described in steps
...
## Run BabaChain Core with CJDNS

Once you are connected to the CJDNS network, the following BabaChain Core
...
When enabled, this option tells BabaChain Core that it is running in an
...
IPv6 local network. This helps BabaChain Core perform better address management:
...
CJDNS support was added to BabaChain Core in version 21.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `babachain-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/babachainpay/babachain/blob/master/doc/tor.md) for
details.

## CJDNS-related information in BabaChain Core

There are several ways to see your CJDNS address in BabaChain Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `babachain-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `babachain-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `babachain-cli help getnodeaddresses` for details.
