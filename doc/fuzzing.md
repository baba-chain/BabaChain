# Fuzzing BabaChain Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing BabaChain Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/baba-chain/babachain
$ cd babachain/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/baba-chain/babachain/blob/develop/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/baba-chain/babachain/blob/develop/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/baba-chain/babachain/blob/develop/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/baba-chain/babachain/tree/develop/src/test/fuzz).
...
It is possible to specify `babachaind` arguments to the `fuzz` executable.
Depending on the test, they may be ignored or consumed and alter the behavior
of the test. Just make sure to use double-babachain to distinguish them from the
...
Every single pull request submitted against the BabaChain Core repo is automatically tested against all inputs in the [`bitcoin-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make BabaChain Core more robust.
...
# Fuzzing BabaChain Core using afl++

## Quickstart guide

To quickly get started fuzzing BabaChain Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing BabaChain Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing BabaChain Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the BabaChain Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as BabaChain
Core without having to write any custom fuzzing harness. The `babachaind` server
...
program, which includes a babachainboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list).
