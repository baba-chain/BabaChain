...
Since version 0.21, BabaChain Core no longer has a default wallet.
...
The following command, for example, creates a descriptor wallet. More information about this command may be found by running `babachain-cli help createwallet`.

```
$ babachain-cli -named createwallet wallet_name="wallet-01" descriptors=true
```
...
| -----------------|:------------------------------------------------------------|
| Linux            | `/home/<user>/.babachaincore/wallets`                             |
| Windows          | `C:\Users\<user>\AppData\Roaming\BabaChainCore\wallets`           |
| macOS            | `/Users/<user>/Library/Application Support/BabaChainCore/wallets` |
...
```
$ babachain-cli -rpcwallet="wallet-01" encryptwallet "passphrase"
```

Once encrypted, the passphrase can be changed with the `walletpassphrasechange` command.

```
$ babachain-cli -rpcwallet="wallet-01" walletpassphrasechange "oldpassphrase" "newpassphrase"
```
...
```
$ babachain-cli -named createwallet wallet_name="wallet-01" descriptors=true passphrase="passphrase"
```
...
```
$ babachain-cli -rpcwallet="wallet-01" sendtoaddress "tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx" 0.01
error code: -13
error message:
...
```
$ babachain-cli -rpcwallet="wallet-01" walletpassphrase "passphrase" 120
```
...
```
$ babachain-cli -rpcwallet="wallet-01" backupwallet /home/node01/Backups/backup-01.dat
```
...
```
$ babachain-cli restorewallet "restored-wallet" /home/node01/Backups/backup-01.dat
```

After that, `getwalletinfo` can be used to check if the wallet has been fully restored.

```
$ babachain-cli -rpcwallet="restored-wallet" getwalletinfo
```
