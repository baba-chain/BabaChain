...
Below are some notes on how to build BabaChain Core for Windows.

The options known to work for building BabaChain Core on Windows are:
...
Note that for WSL the BabaChain Core source path MUST be somewhere in the default mount file system, for
example /usr/src/babachain, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\babachain`, for example:

    make install DESTDIR=/mnt/c/workspace/babachain
