# User-space, Statically Defined Tracing (USDT) for BabaChain Core

BabaChain Core includes statically defined tracepoints to allow for more
...
**Note**: BabaChain Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to BabaChain Core
...
Multiple tools can list the available tracepoints in a `babachaind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in BabaChain Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/babachaind
stap net        outbound_message 0x0000000000107c05 /src/babachaind
stap validation block_connected  0x00000000002fb10c /src/babachaind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in BabaChain Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/babachaind | grep NT_STAPSDT -A 4 -B 2
