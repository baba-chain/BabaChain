...
- [Development guidelines](#development-guidelines)
    - [General BabaChain Core](#general-babachain-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BabaChain Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while babachaind is running with the `logging`
RPC.  For instance, launching babachaind with `-debug` or `-debug=1` will turn on
...
BabaChain Core is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run babachaind.

You can then use the `-debug=lock` configuration option at babachaind startup or
`babachain-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `babachain-cli logging [] '["lock"]'`.
...
([`valgrind.supp`](https://github.com/babachainpay/babachain/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_babachain
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_babachain --log_level=test_suite
$ valgrind -v --leak-check=full src/babachaind -printtoconsole
...
# A coverage report will now be accessible at `./test_babachain.coverage/index.html`.
```
...
To profile a running babachaind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep babachaind` -- sleep 60
```
...
BabaChain Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`babachaind`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
However, in open source software such as BabaChain Core, where everyone uses
...
pay attention to for reviewers of BabaChain Core code.

General BabaChain Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: BabaChain Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of BabaChain Core, in which case
...
a BabaChain Core subtree, so that they can be integrated quickly. Cosmetic changes
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `babachain-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `babachain-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of BabaChain Core, the wallet was always
...
- Use *invalid* BabaChain addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
