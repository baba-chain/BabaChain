...
Before starting this tutorial, start the babachain node on the testnet network.

```bash
./src/babachaind -testnet -daemon
```
...
At the time of writing, there is no way to extract a specific path from wallets in BabaChain Core. For this, an external signer/xpub can be used.
...
for ((n=1;n<=3;n++))
do
 ./src/babachain-cli -testnet -named createwallet wallet_name="participant_${n}" descriptors=true
done
```

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, BabaChain Core single-sig wallets are created using path `m/44'/1'/0'/` for PKH based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in BabaChain Core will include the checksum in their output.
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./src/babachain-cli -testnet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("pkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./src/babachain-cli -testnet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("pkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```
...
external_desc_sum=$(./src/babachain-cli -testnet getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./src/babachain-cli -testnet getdescriptorinfo $internal_desc | jq '.descriptor')
...
Documentation for these and other parameters can be found by typing `./src/babachain-cli help importdescriptors`.
...
```bash
./src/babachain-cli -testnet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true descriptors=true

./src/babachain-cli  -testnet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./src/babachain-cli  -testnet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./src/babachain-cli -testnet loadwallet "participant_${n}"; done
```
...
```bash
receiving_address=$(./src/babachain-cli -testnet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/testnet/getcoins.py -c ./src/babachain-cli -a $receiving_address
```
...
```bash
./src/babachain-cli -testnet -rpcwallet="multisig_wallet_01" getbalances
```
...
PSBT is a data format that allows wallets and other tools to exchange information about a BabaChain transaction and the signatures necessary to complete it. [[source](https://bitcoinops.org/en/topics/psbt/)]

The current PSBT version (v0) is defined in [BIP 174](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki).

For simplicity, the destination address is taken from the `participant_1` wallet in the code above, but it can be any valid babachain address.
...
```bash
balance=$(./src/babachain-cli -testnet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./src/babachain-cli -testnet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./src/babachain-cli -testnet -named -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```
...
```bash
./src/babachain-cli -testnet decodepsbt $funded_psbt

./src/babachain-cli -testnet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./src/babachain-cli -testnet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./src/babachain-cli -testnet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./src/babachain-cli -testnet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./src/babachain-cli -testnet finalizepsbt $combined_psbt | jq -r '.hex')

./src/babachain-cli -testnet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./src/babachain-cli -testnet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./src/babachain-cli -testnet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./src/babachain-cli -testnet finalizepsbt $psbt_2 | jq -r '.hex')
