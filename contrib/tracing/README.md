...
Tracing (USDT) support for BabaChain Core on Linux using. For more information on
USDT support in BabaChain Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `babachaind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `babachaind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[BabaChain Core USDT documentation] on how to list available tracepoints in your
`babachaind` binary.

[BabaChain Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/babachaind
```
...
BabaChain P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/babachaind
```
...
In a different terminal, starting BabaChain Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/babachaind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/babachaind
```
