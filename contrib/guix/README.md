# Bootstrappable BabaChain Core Builds

This directory contains the files necessary to perform bootstrappable BabaChain
Core builds.
...
To build BabaChain Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      BabaChain Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/babachain-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/babachainpay/guix.sigs).
