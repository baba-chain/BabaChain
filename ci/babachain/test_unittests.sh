...
cd "build-ci/babachaincore-$BUILD_TARGET"
...
if [ "$DIRECT_WINE_EXEC_TESTS" = "true" ]; then
  ...
  wine ./src/test/test_babachain.exe
else
  if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
    ./src/test/test_babachain --catch_system_errors=no -l test_suite
  else
