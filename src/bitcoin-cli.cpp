...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-generate",
                   strprintf("Generate blocks, equivalent to RPC getnewaddress followed by RPC generatetoaddress. Optional positional integer "
                             ...
                             "RPC generatetoaddress nblocks and maxtries arguments. Example: babachain-cli -generate 4 1000",
    ...
    argsman.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to babachaind). This changes the RPC endpoint used, e.g. http://127.0.0.1:9998/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (gArgs.IsArgSet("-version")) {
            ...
            strUsage += "\n"
                "Usage:  babachain-cli [options] <command> [params]  Send command to " PACKAGE_NAME "\n"
                "or:     babachain-cli [options] -named <command> [name=value]...  Send command to " PACKAGE_NAME " (with named arguments)\n"
                "or:     babachain-cli [options] help                List commands\n"
                "or:     babachain-cli [options] help <command>      Get help for a command\n";
...
public:
    ...
    UniValue ProcessReply(const UniValue& reply) override
    {
        ...
        if (!nodes.empty() && nodes.at(0)["network"].isNull()) {
            throw std::runtime_error("-addrinfo requires babachaind server to be running v21.0 and up");
        }
...
private:
    ...
    uint8_t m_details_level{0}; //!< Optional user-supplied arg to set babachainboard details level
...
public:
    ...
    UniValue PrepareRequest(const std::string& method, const std::vector<std::string>& args) override
    {
        if (!args.empty()) {
            ...
            if (ParseUInt8(args.at(0), &n)) {
                ...
                throw std::runtime_error(strprintf("invalid -netinfo argument: %s\nFor more information, run: babachain-cli -netinfo help", args.at(0)));
            }
        }
        UniValue result(UniValue::VARR);
    ...
    UniValue ProcessReply(const UniValue& batch_in) override
    {
        ...
        if (networkinfo["version"].getInt<int>() < 200000) {
            throw std::runtime_error("-netinfo requires babachaind server to be running v20.0 and up");
        }
    ...
    const std::string m_help_doc{
        "-netinfo level|\"help\" \n\n"
        "Returns a network peer connections babachainboard with information from the remote server.\n"
        ...
        "Suggestion: use with the Linux watch(1) command for a live babachainboard; see example below.\n\n"
        "Arguments:\n"
        + strprintf("1. level (integer 0-%d, optional)  Specify the info level of the peers babachainboard (default 0):\n", MAX_DETAIL_LEVEL) +
        ...
        "2. help (string \"help\", optional) Print this help documentation instead of the babachainboard.\n\n"
        "Result:\n\n"
        ...
        "> babachain-cli -netinfo\n\n"
        "The same, preceded by a peers listing without address and version columns\n"
        "> babachain-cli -netinfo 1\n\n"
        "Full babachainboard\n"
        + strprintf("> babachain-cli -netinfo %d\n\n", MAX_DETAIL_LEVEL) +
        "Full live babachainboard, adjust --interval or --no-title as needed (Linux)\n"
        + strprintf("> watch --interval 1 --no-title babachain-cli -netinfo %d\n\n", MAX_DETAIL_LEVEL) +
        "See this help\n"
        "> babachain-cli -netinfo help\n"};
};
...
static UniValue CallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::optional<std::string>& rpcwallet = {})
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\n"
                    "Make sure the babachaind server is running and that you are connecting to the correct RPC port.\n"
                    "Use \"babachain-cli -help\" for more info.",
...
static void ParseError(const UniValue& error, std::string& strPrint, int& nRet)
{
    if (error.isObject()) {
        ...
        if (err_code.isNum() && err_code.getInt<int>() == RPC_WALLET_NOT_SPECIFIED) {
            strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to babachain-cli command line.";
        }
    } else {
