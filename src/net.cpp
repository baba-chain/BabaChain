...
const std::array<std::string, 33> V2_BITCOIN_IDS = {
    ...
    "", /* FEEFILTER is not implemented in BabaChain */
...
/** List of short messages allocated in BabaChain's reserved namespace, in order.
 *
 ...
 */
const std::array<std::string, 40> V2_BABACHAIN_IDS = {
    NetMsgType::SPORK,
 ...
 *
 * Bitcoin takes up short IDs up to 128 (lower half) while BabaChain can take
...
bool IsValidV2ShortID(uint8_t first_byte) {
    ...
    // and BabaChain's *used* slots. We do this by checking if the byte is within
    // the range where a valid message is expected to reside.
    return first_byte < std::size(V2_BITCOIN_IDS) ||
           (first_byte >= 128 && static_cast<uint8_t>(first_byte - 128) < std::size(V2_BABACHAIN_IDS));
}
