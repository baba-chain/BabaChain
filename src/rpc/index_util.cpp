...
static void EnsureAddressIndexAvailable()
{
    if (!node::fAddressIndex) {
        throw JSONRPCError(RPC_INVALID_REQUEST, "Address index is disabled. You should run BabaChain Core with -addressindex (requires reindex)");
    }
}
                   ...
                   CSpentIndexValue& value)
{
    ...
    if (!node::fSpentIndex) {
        throw JSONRPCError(RPC_INVALID_REQUEST, "Spent index is disabled. You should run BabaChain Core with -spentindex (requires reindex)");
    }
                       ...
                       std::vector<uint256>& hashes)
{
    ...
    if (!node::fTimestampIndex) {
        throw JSONRPCError(RPC_INVALID_REQUEST, "Timestamp index is disabled. You should run BabaChain Core with -timestampindex (requires reindex)");
    }
