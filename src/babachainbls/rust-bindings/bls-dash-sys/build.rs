...
fn main() {
    ...
    let bls_babachain_build_path = root_path.join("build");
    let bls_babachain_src_path = root_path.join("src");
    let c_bindings_path = root_path.join("rust-bindings/bls-babachain-sys/c-bindings");

    println!("root {}", root_path.display());
    println!("bls_babachain_build_path {}", bls_babachain_build_path.display());
    println!("bls_babachain_src_path {}", bls_babachain_src_path.display());
    ...
    if bls_babachain_build_path.exists() {
        fs::remove_dir_all(&bls_babachain_build_path).expect("can't clean build directory");
    }

    fs::create_dir_all(&bls_babachain_build_path).expect("can't create build directory");
    ...
    cmake_command
        .current_dir(&bls_babachain_build_path)
    ...
    let include_paths_file_path = bls_babachain_build_path.join("include_paths.txt");
    ...
    include_paths.extend([
        bls_babachain_build_path.join("depends/relic-src/include"),
        bls_babachain_build_path.join("depends/relic/include"),
        bls_babachain_build_path.join("src"),
        root_path.join("include/babachainbls"),
        bls_babachain_build_path.join("depends/relic/include"),
        bls_babachain_build_path.join("depends/mimalloc/include"),
        root_path.join("depends/relic/include"),
        root_path.join("depends/mimalloc/include"),
        bls_babachain_src_path.clone(),
    ]);
    ...
    cc.compile("bls-babachain-sys");

    // // Link dependencies
    // println!(
    //     "cargo:rustc-link-search={}",
    //     bls_babachain_build_path.join("_deps/sodium-build").display()
    // );
    ...
    println!(
        "cargo:rustc-link-search={}",
        bls_babachain_build_path.join("src").display()
    );

    println!("cargo:rustc-link-lib=static=babachainbls");

    // Link GMP if exists
    let gmp_libraries_file_path = bls_babachain_build_path.join("gmp_libraries.txt");
    ...
    println!("cargo:warning=########## bls_babachain_build_path:{}", bls_babachain_build_path.display());
    ...
    println!("cargo:rerun-if-changed={}", bls_babachain_src_path.display());
}
...
//     let bls_babachain_build_path = root_path.join("build");
//     let bls_babachain_src_path = root_path.join("src");
//     let artefacts_path = bls_babachain_build_path.join("artefacts");
...
//     println!("cargo:rustc-link-lib=static=sodium");
//     println!("cargo:rustc-link-lib=static=relic_s");
//     println!("cargo:rustc-link-search={}", bls_babachain_build_path.join("src").display());
...
fn main() {
    ...
    let bls_babachain_build_path = root_path.join("build");
    let bls_babachain_src_path = root_path.join("src");
    let bls_babachain_src_include_path = root_path.join("include/babachainbls");
    let c_bindings_path = root_path.join("rust-bindings/bls-babachain-sys/c-bindings");
    let artefacts_path = bls_babachain_build_path.join("artefacts");
    let target_path = artefacts_path.join(&target);
    ...
    include_paths.extend([
        bls_babachain_build_path.join(format!("relic-{}-{}/depends/relic-src/include", platform, arch)),
        bls_babachain_build_path.join(format!("relic-{}-{}/depends/relic/include", platform, arch)),
        bls_babachain_build_path.join("contrib/relic/src"),
        root_path.join("src"),
        root_path.join("include/babachainbls"),
        root_path.join("depends/relic/include"),
        root_path.join("depends/mimalloc/include"),
        ...
        bls_babachain_src_path.clone(),
        bls_babachain_src_include_path.clone()
    ]);
    ...
    println!("cargo:rustc-link-lib=static=relic_s");
    println!("cargo:rustc-link-lib=static=bls");
    println!("cargo:rustc-link-search={}", bls_babachain_src_path.display());
    println!("cargo:rustc-link-lib=static=babachainbls");
    println!("cargo:rerun-if-changed={}", bls_babachain_src_path.display());
}
