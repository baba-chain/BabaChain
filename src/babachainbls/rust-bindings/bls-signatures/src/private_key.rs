...
impl PrivateKey {
    ...
    pub fn generate_babachain(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "babachain_helpers")]
    pub fn generate_babachain_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_babachain(rng))
            .collect()
    }
