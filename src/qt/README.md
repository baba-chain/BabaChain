This directory contains the source code for the BabaChain Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the BabaChain UI.
...
- ClientModel is used by the main application `babachaingui` and several models like `peertablemodel`.
...
#### paymentserver.(h/cpp)
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a babachain:... link from a browser).

Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a babachain:... link from a browser).
...
**Note:** Do not change `local/babachain_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `babachain-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `babachain-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `babachain-qt`
9. You're all set! Start developing, building, and debugging the BabaChain Core GUI
