...
const char * const BITCOIN_CONF_FILENAME = "babachain.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //babachain-tx using stdin
        ...
        if (key[0] != '-') {
            if (!m_accept_any_command && m_command.empty()) {
                // The first non-babachain arg is a registered command
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BabaChainCore
    // macOS: ~/Library/Application Support/BabaChainCore
    // Unix-like: ~/.babachaincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BabaChainCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BabaChainCore";
#else
    // Unix-like
    return pathRet / ".babachaincore";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty babachain.conf if it does not exist
