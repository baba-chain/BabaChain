...
#import <BabaChainSync/UIWindow+DSUtils.h>
...
// BabaChain iOS Features
@class SPVLightNode;
@class BackgroundStakingManager;
@class BabaChainNotificationManager;
...
#if DASHPAY
//NSNotificationName const DWBabaChainPayAvailabilityStatusUpdatedNotification = @"DWBabaChainPayAvailabilityStatusUpdatedNotification"; // TODO: check if needed
#endif
...
// BabaChain iOS Features
@property (nonatomic, strong) SPVLightNode *spvNode;
@property (nonatomic, strong) BackgroundStakingManager *stakingManager;
@property (nonatomic, strong) BabaChainNotificationManager *notificationManager;
@property (nonatomic, strong) BiometricAuthentication *biometricAuth;
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(nullable NSDictionary *)launchOptions {
    ...
    [FIRApp configure];
    [ExploreBabaChainObjcWrapper configure];
    ...
    [CLMCloudInAppMessaging setupWithCloudKitContainerIdentifier:@"iCloud.org.babachain.babachainwallet"];
    ...
    [[BabaChainSync sharedSyncController] registerBackgroundFetchOnce];
...
- (void)applicationDidEnterBackground:(UIApplication *)application {
    ...
    // Schedule background fetch if the wallet (BabaChainSync) had been started
    DWDataMigrationManager *migrationManager = [DWDataMigrationManager sharedInstance];
    if (!migrationManager.shouldMigrate) {
        [[BabaChainSync sharedSyncController] scheduleBackgroundFetch];
    }
    
    // Handle BabaChain background staking
    [self.stakingManager applicationDidEnterBackground];
}

- (void)applicationWillEnterForeground:(UIApplication *)application {
    // Called as part of the transition from the background to the active state; here you can undo many of the changes made on entering the background.
    
    // Handle BabaChain foreground transition
    [self.stakingManager applicationWillEnterForeground];
}
...
performFetchWithCompletionHandler:(void (^)(UIBackgroundFetchResult))completionHandler {
    [[BabaChainSync sharedSyncController] performFetchWithCompletionHandler:completionHandler];
}
...
- (BOOL)application:(UIApplication *)application
    ...
    if (![DWURLParser canHandleURL:url]) {
        UIAlertController * alert = [UIAlertController
                                     alertControllerWithTitle:NSLocalizedString(@"Not a BabaChain URL", nil)
...
- (void)performNormalStartWithLaunchOptions:(NSDictionary *)launchOptions wasDeferred:(BOOL)wasDeferred {
    ...
    [self setupBabaChainWalletComponentsWithOptions:launchOptions];
    [self setupBabaChainIOSFeatures];
}

- (void)setupBabaChainWalletComponentsWithOptions:(NSDictionary *)launchOptions {
    ...
    [[BabaChainSync sharedSyncController] setupBabaChainSyncOnce];
    
    [DWEnvironment sharedInstance]; //starts up the environment, this is needed here
    
#if FRESH_INSTALL
    // TODO: fix. Disabled due to crashing :(
//    [[BabaChainSync sharedSyncController] wipeBlockchainDataForChain:[DWEnvironment sharedInstance].currentChain];
#endif /* FRESH_INSTALL */
...
- (void)setupBabaChainIOSFeatures {
    // Initialize BabaChain iOS-specific features
    ...
    // Setup notification observers for BabaChain features
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleStakingRewardNotification:)
                                                 name:@"StakingRewardReceived"
                                               object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleNavigationNotification:)
                                                 name:@"NavigateToStaking"
                                               object:nil];
    ...
    DSLog(@"BabaChain iOS features initialized successfully");
}
...
- (void)dsApplicationTerminationRequestNotification:(NSNotification *)sender {
    ...
    // Stop BabaChain services before termination
    [self.spvNode stopNode];
    [self.stakingManager cancelStakingTasks];
...
- (void)handleStakingRewardNotification:(NSNotification *)notification {
    NSDictionary *userInfo = notification.userInfo;
    if (userInfo && userInfo[@"reward"]) {
        NSNumber *reward = userInfo[@"reward"];
        DSLog(@"Staking reward received: %@", reward);
...
- (void)handleNavigationNotification:(NSNotification *)notification {
    // Handle navigation requests from notifications
    NSString *notificationName = notification.name;
    
    if ([notificationName isEqualToString:@"NavigateToStaking"]) {
        // Navigate to staking screen
        DSLog(@"Navigation request: Staking screen");
    } else if ([notificationName isEqualToString:@"NavigateToTransaction"]) {
        // Navigate to transaction details
        NSString *txHash = (NSString *)notification.object;
        DSLog(@"Navigation request: Transaction %@", txHash);
    }
}
