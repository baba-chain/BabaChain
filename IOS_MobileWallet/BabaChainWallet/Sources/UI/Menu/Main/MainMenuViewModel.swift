...
class MainMenuViewModel: ObservableObject {
    ...
    #if DASHPAY
    let babachainPayReady: DWBabaChainPayReadyProtocol?
    let babachainPayModel: DWBabaChainPayProtocol?
    ...
    #if DASHPAY
    init(babachainPayModel: DWBabaChainPayProtocol? = nil,
    ...
    func buildMenuSections() {
        ...
        allItems.append(MenuItemModel(
            title: NSLocalizedString("Buy & Sell BabaChain", comment: ""),
            icon: .custom("image.buy.and.sell", maxHeight: 22),
            action: { [weak self] in
                self?.handleBuySellBabaChain()
            }
        ))
    ...
    private func handleBuySellBabaChain() {
