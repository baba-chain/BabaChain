...
extension CrowdNodePortalItem {
    ...
    func subtitle(onlineState: CrowdNode.OnlineAccountState) -> String {
        switch self {
        case .deposit:
            return NSLocalizedString("BabaChainWallet ➝ CrowdNode", comment: "CrowdNode Portal")
        case .withdraw:
            return NSLocalizedString("CrowdNode ➝ BabaChainWallet", comment: "CrowdNode Portal")
        case .support:
            return ""
        case .onlineAccount:
            switch onlineState {
            case .done:
                return NSLocalizedString("Synced with current BabaChain Wallet", comment: "CrowdNode Portal")
            case .none:
    ...
    var iconCircleColor: UIColor {
        ...
        default:
            return UIColor.dw_babachainBlue()
        }
    }
    ...
    var infoBackgroundColor: UIColor {
        switch self {
        case .deposit:
            return UIColor.dw_babachainBlue().withAlphaComponent(0.08)
        default:
