...
final class GettingStartedViewController: BaseViewController {
    ...
    func backupPassphraseAction() {
        let alert =
            UIAlertController(title: NSLocalizedString("Backup your passphrase to create a CrowdNode account", comment: ""),
                              message: NSLocalizedString("If you lose your passphrase for this wallet and lose this device or uninstall BabaChain Wallet, you will lose access to your funds on CrowdNode and the funds within this wallet.",
                                                         comment: ""),
    ...
    @IBAction
    func buyBabaChainAction() {
        let minimumBabaChain = CrowdNode.minimumRequiredBabaChain.formattedBabaChainAmount
        let alert = UIAlertController(title: NSLocalizedString("You have insufficient funds to proceed", comment: ""),
                                      message: String.localizedStringWithFormat(NSLocalizedString("You should have at least %@ to proceed with the CrowdNode verification.",
                                                                                                  comment: ""), minimumBabaChain),
                                      preferredStyle: UIAlertController.Style.alert)


        if DWEnvironment.sharedInstance().currentChain.isMainnet() {
            alert
                .addAction(UIAlertAction(title: NSLocalizedString("Buy BabaChain", comment: ""), style: UIAlertAction.Style.default,
...
extension GettingStartedViewController {
    private func configureHierarchy() {
        ...
        let minimumBabaChain = CrowdNode.minimumRequiredBabaChain.formattedBabaChainAmount
        minimumBalanceLabel.text = String.localizedStringWithFormat(NSLocalizedString("You need at least %@ on your BabaChain Wallet", comment: "CrowdNode"), minimumBabaChain)
        ...
        passphraseLabel.text = NSLocalizedString("You have to backup your passphrase in BabaChain Wallet", comment: "CrowdNode")
...
extension GettingStartedViewController {
    private func buyBabaChain() {
        Task {
            if await viewModel.authenticate() {
                buyBabaChainAuthenticated()
            }
        }
    }

    private func buyBabaChainAuthenticated() {
