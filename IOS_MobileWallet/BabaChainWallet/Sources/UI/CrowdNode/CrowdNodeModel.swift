...
final class CrowdNodeModel {
    ...
    var buyBabaChainButtonText: String {
        if DWEnvironment.sharedInstance().currentChain.isMainnet() {
            return NSLocalizedString("Buy BabaChain", comment: "CrowdNode")
        } else {
            return ""
        }
    }
    ...
    var withdrawalLimits: [Int] { [
        Int(prefs.crowdNodeWithdrawalLimitPerTx / kOneBabaChain),
        Int(prefs.crowdNodeWithdrawalLimitPerHour / kOneBabaChain),
        Int(prefs.crowdNodeWithdrawalLimitPerDay / kOneBabaChain),
    ] }
...
extension CrowdNodeModel {
    ...
    private func checkBalance() {
        ...
        hasEnoughWalletBalance = walletBalance >= CrowdNode.minimumRequiredBabaChain
    }
}
