...
class ExploreViewController: UIViewController, NavigationFullscreenable {
    ...
    private func setupLayout() {
        ...
        headerView.image = UIImage(named: "image.explore.babachain.wallet")
        headerView.title = NSLocalizedString("Explore BabaChain", comment: "")
        headerView.subtitle = NSLocalizedString("Find merchants that accept BabaChain, where to buy it and how to earn income with it.", comment: "")
        ...
        contentsView.getTestBabaChainHandler = { [weak self] in
            self?.getTestBabaChainAction()
        }
    ...
    private func getTestBabaChainAction() {
        ...
        if let url = URL(string: "http://faucet.testnet.networks.babachain.org/") {
    ...
    private func showAtms() {
        ...
        vc.payWithBabaChainHandler = { [weak self] in
            guard let self = self else { return }
            self.delegate?.exploreViewControllerShowReceivePayment(self)
        }
        vc.sellBabaChainHandler = { [weak self] in
            guard let self = self else { return }
