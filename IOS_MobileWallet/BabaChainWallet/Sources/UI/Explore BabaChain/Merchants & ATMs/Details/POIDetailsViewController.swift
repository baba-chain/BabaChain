...
class POIDetailsViewController: UIViewController {
    ...
    @objc public var payWithBabaChainHandler: (()->())?
    @objc var sellBabaChainHandler: (()->())?
...
extension POIDetailsViewController {
    ...
    private func showDetailsView() {
        ...
        detailsView.payWithBabaChainHandler = payWithBabaChainHandler
        detailsView.sellBabaChainHandler = sellBabaChainHandler
        detailsView.showAllLocationsActionBlock = { [weak self] in
            ...
            vc.payWithBabaChainHandler = wSelf.payWithBabaChainHandler
            vc.sellBabaChainHandler = wSelf.sellBabaChainHandler
            wSelf.navigationController?.pushViewController(vc, animated: true)
        }
        detailsView.buyGiftCardHandler = { [weak self] provider in
            self?.showBabaChainSpendPayScreen(provider: provider)
        }
        detailsView.babachainSpendAuthHandler = { [weak self] provider in
            self?.showBabaChainSpendLoginInfo(provider: provider)
        }
...
extension POIDetailsViewController {
    private func showBabaChainSpendLoginInfo(provider: GiftCardProvider) {
        let swiftUIView = BabaChainSpendLoginInfoView(
            provider: provider,
            onCreateNewAccount: { [weak self] in
                self?.dismiss(animated: true) {
                    self?.showBabaChainSpendTerms(provider: provider)
                }
            },
            onLogIn: { [weak self] in
                self?.dismiss(animated: true) {
                    self?.showBabaChainSpendAuth(authType: .signIn, provider: provider)
                }
            },
    ...
    private func showBabaChainSpendTerms(provider: GiftCardProvider) {
        let hostingController = UIHostingController(
            rootView: BabaChainSpendTermsScreen(provider: provider) {
                self.navigationController?.popToViewController(ofType: POIDetailsViewController.self, animated: false)
                self.showBabaChainSpendPayScreen(provider: provider, justAuthenticated: true)
            }
        )
    ...
    private func showBabaChainSpendAuth(authType: BabaChainSpendUserAuthType, provider: GiftCardProvider) {
        let hostingController = UIHostingController(
            rootView: BabaChainSpendUserAuthScreen(authType: authType, provider: provider) {
    ...
    private func showBabaChainSpendPayScreen(provider: GiftCardProvider, justAuthenticated: Bool = false) {
        let hostingController = UIHostingController(
            rootView: BabaChainSpendPayScreen(merchant: self.pointOfUse, provider: provider, justAuthenticated: justAuthenticated) { [weak self] txId in
    ...
    private func tryRefreshCtxToken() async throws -> Bool {
        do {
            ...
            return true
        } catch BabaChainSpendError.tokenRefreshFailed {
            await showModalDialog(style: .warning, icon: .system("exclamationmark.triangle.fill"), heading: NSLocalizedString("Your session expired", comment: "BabaChainSpend"), textBlock1: NSLocalizedString("It looks like you haven’t used BabaChainSpend in a while. For security reasons, you’ve been logged out.\n\nPlease sign in again to continue exploring where to spend your BabaChain.", comment: "BabaChainSpend"), positiveButtonText: NSLocalizedString("Dismiss", comment: ""))
            return false
        }
    }
}
