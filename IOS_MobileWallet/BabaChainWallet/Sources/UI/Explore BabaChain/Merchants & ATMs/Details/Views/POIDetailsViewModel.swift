...
class POIDetailsViewModel: ObservableObject, SyncingActivityMonitorObserver, NetworkReachabilityHandling, DWLocationObserver {
    ...
    func observeBabaChainSpendState(provider: GiftCardProvider?) {
    ...
    func selectProvider(_ provider: GiftCardProvider) {
        ...
        observeBabaChainSpendState(provider: provider)
    }
    ...
    private func updateDistance() {
        ...
        distanceText = ExploreBabaChain.distanceFormatter.string(from: measurement)
    }
    ...
    private func setupProviders() {
        ...
        if let selectedProvider = selectedProvider {
            observeBabaChainSpendState(provider: selectedProvider)
        }
    }
    ...
    private func fetchLocationCount() {
        ...
        if shouldIgnoreRadius {
            ...
            ExploreBabaChain.shared.allLocations(for: merchant.pointOfUseId, in: nil, userPoint: nil) { [weak self] result in
                Task { @MainActor in
                    switch result {
        ...
        ExploreBabaChain.shared.allLocations(for: merchant.pointOfUseId, in: bounds, userPoint: currentLocation.coordinate) { [weak self] result in
            Task { @MainActor in
                switch result {
                case .success(let locations):
                    let count = locations.items.count
