        ...
        do {
            ...
            case "fulfilled":
                if let cardNumber = response.cardNumber, !cardNumber.isEmpty {
                    ...
                    stopTicker()
                    DSLogger.log("BabaChainSpend: Gift card details fetched successfully")
                }
                
            case "rejected":
                await MainActor.run {
                    self.uiState.loadingError = BabaChainSpendError.customError(
            ...
            DSLogger.log("BabaChainSpend: Failed to fetch gift card info: \(error)")
        }
    }
