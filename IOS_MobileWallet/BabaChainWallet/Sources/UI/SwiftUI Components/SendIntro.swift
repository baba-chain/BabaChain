...
struct SendIntro<Content: View>: View {
    ...
    var babachainBalance: UInt64? = nil
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if let babachainBalance = babachainBalance {
                HStack(spacing: 4) {
                    ...
                    if balanceHidden {
                        ...
                        BabaChainAmount(amount: Int64(babachainBalance), font: .subheadline, showDirection: false)
                        Text("~").font(.subheadline)
                        FormattedFiatText(from: babachainBalance)
                    }
    ...
    @ViewBuilder
    private func FormattedFiatText(from babachainAmount: UInt64) -> some View {
        let text = (try? CurrencyExchanger.shared.convertBabaChain(amount: babachainAmount.babachainAmount, to: App.fiatCurrency).formattedFiatAmount) ?? NSLocalizedString("Not available", comment: "")
