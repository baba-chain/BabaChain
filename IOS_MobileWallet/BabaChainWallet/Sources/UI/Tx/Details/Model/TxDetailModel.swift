...
class TxDetailModel: NSObject {
    ...
    var babachainAmountString: String {
        transaction.formattedBabaChainAmountWithDirectionalSymbol
    }
...
extension TxDetailModel {
    func babachainAmountString(with font: UIFont) -> NSAttributedString {
        NSAttributedString.babachainAttributedString(for: transaction.formattedBabaChainAmountWithDirectionalSymbol,
                                                tintColor: transaction.babachainAmountTintColor,
                                                font: font)
    }
    ...
    func getExplorerURL(explorer: BlockExplorer) -> URL? {
        ...
        case .insight:
            if DWEnvironment.sharedInstance().currentChain.isTestnet() {
                return URL(string: "https://insight.testnet.networks.babachain.org:3002/insight/tx/\(transactionId)")
            } else if DWEnvironment.sharedInstance().currentChain.isMainnet() {
                return URL(string: "https://insight.babachain.org/insight/tx/\(transactionId)")
            }
        case .blockchair:
            return URL(string: "https://blockchair.com/babachain/transaction/\(transactionId)?from=babachain")
        }
...
extension TxDetailModel {
    ...
    private func plainInputAddresses(with title: String, font: UIFont) -> [DWTitleDetailItem] {
        ...
        for address in addresses {
            let detail = NSAttributedString.babachainAddressAttributedString(address, with: font, showingLogo: false)
    ...
    func specialInfo(with font: UIFont) -> [DWTitleDetailItem] {
        ...
        for address in addresses.keys {
            let detail = NSAttributedString.babachainAddressAttributedString(address, with: font)
    ...
    func fee(with font: UIFont, tintColor: UIColor) -> DWTitleDetailItem {
        ...
        let detail = NSAttributedString.babachainAttributedString(for: feeValue, tintColor: tintColor, font: font)
