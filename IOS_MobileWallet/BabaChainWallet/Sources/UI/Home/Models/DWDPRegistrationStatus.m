    ...
    if (self.failed) {
        switch (self.state) {
            ...
            case DWDPRegistrationState_CreatingID:
                return NSLocalizedString(@"(2/3) Unable to create BabaChain Identity", nil);
        ...
        switch (self.state) {
            ...
            case DWDPRegistrationState_CreatingID:
                return NSLocalizedString(@"(2/3) Creating BabaChain Identity", nil);
            case DWDPRegistrationState_RegistrationUsername:
                return NSLocalizedString(@"(3/3) Registering Username", nil);
            case DWDPRegistrationState_Done:
                return NSLocalizedString(@"Your BabaChainPay Username is ready to use", nil);
        }
    }
}
