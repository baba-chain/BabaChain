...
extension HomeViewController: DWRecoverViewControllerDelegate {
    func performJailbreakCheck() {
        ...
        if !model.isWalletEmpty {
            message = NSLocalizedString("DEVICE SECURITY COMPROMISED\nAny 'jailbreak' app can access any other app's keychain data (and steal your BabaChain). Wipe this wallet immediately and restore on a secure device.", comment: "")
            mainAction = UIAlertAction(title: NSLocalizedString("Wipe", comment: ""), style: .destructive) { [weak self] action in
                self?.wipeWallet()
            }
        } else {
            message = NSLocalizedString("DEVICE SECURITY COMPROMISED\nAny 'jailbreak' app can access any other app's keychain data (and steal your BabaChain).", comment: "")
