...
class HomeViewModel: ObservableObject {
    ...
    @Published private(set) var showJoinBabaChainpay: Bool = true
...
#if DASHPAY
    var joinBabaChainPayState: JoinBabaChainPayState = .callToAction
#endif
    ...
    var shouldShowMixBabaChainDialog: Bool {
        get { coinJoinService.mode == .none || !UsernamePrefs.shared.mixBabaChainShown }
        ...
        if walletNeedsBackup {
            ...
            if userHasBalance {
                ...
                if DWEnvironment.sharedInstance().currentChain.isMainnet() {
                    mutableItems.append(ShortcutAction(type: .buySellBabaChain))
                }
            }
        } else {
            if userHasBalance {
...
#if DASHPAY
extension HomeViewModel {
    func checkJoinBabaChainPay() {
        self.showJoinBabaChainpay = syncModel.state == .syncDone &&
            !UsernamePrefs.shared.joinBabaChainPayDismissed &&
            joinBabaChainPayState != .voting && joinBabaChainPayState != .registered
    }
    
    private func observeBabaChainPay() {
        NotificationCenter.default.publisher(for: .DWBabaChainPayRegistrationStatusUpdated)
            .sink { [weak self] _ in
                self?.checkJoinBabaChainPay()
                self?.reloadTxsAndShortcuts()
