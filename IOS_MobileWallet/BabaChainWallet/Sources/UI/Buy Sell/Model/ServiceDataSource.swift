...
class UpholdDataSource: ServiceDataSource {
    private var babachainCard: DWUpholdCardObject!
    ...
    override func refresh() {
        if DWUpholdClient.sharedInstance().isAuthorized {
            if let balance = DWUpholdClient.sharedInstance().lastKnownBalance as? Decimal {
                ...
                self.babachainBalance = balance.plainBabaChainAmount
            } else {
                DWUpholdClient.sharedInstance().getCards { [weak self] babachainCard, _ in
                    self?.babachainCard = babachainCard

                    if let available = babachainCard?.available as? Decimal {
                        self?.status = .authorized
                        self?.babachainBalance = available.plainBabaChainAmount
                    } else {
                        self?.status = .failed
                    }
                }
