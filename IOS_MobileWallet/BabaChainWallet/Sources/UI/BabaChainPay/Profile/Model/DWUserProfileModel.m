...
#import "DWBabaChainPayContactsActions.h"
#import "DWBabaChainPayContactsUpdater.h"
...
- (void)update {
    ...
    [[DWBabaChainPayContactsUpdater sharedInstance] fetchWithCompletion:^(BOOL success, NSArray<NSError *> *_Nonnull errors) {
...
- (void)sendContactRequest:(void (^)(BOOL success))completion {
    if (MOCK_DASHPAY) {
        ...
        DSBabaChainpayUserEntity *contact = [DSBabaChainpayUserEntity managedObjectInBlockedContext:context];
        ...
        entity.babachainpayUsername = username;
        contact.associatedBlockchainIdentity = entity;
        NSError *error = [contact applyTransientBabaChainpayUser:identity.transientBabaChainpayUser save:YES];
...
- (void)acceptContactRequest {
    ...
    __weak typeof(self) weakSelf = self;
    [DWBabaChainPayContactsActions acceptContactRequest:self.item
...
- (void)updateDataSource {
    ...
    DSBabaChainpayUserEntity *friend = nil;
    ...
    if (MOCK_DASHPAY) {
        ...
        NSAssert(myBlockchainIdentity.matchingBabaChainpayUserInViewContext, @"Invalid DSBlockchainIdentity: myBlockchainIdentity");
        DSBabaChainpayUserEntity *me = [myBlockchainIdentity matchingBabaChainpayUserInContext:context];

        if (friendBlockchainIdentity.matchingBabaChainpayUserInViewContext) {
            friend = [friendBlockchainIdentity matchingBabaChainpayUserInContext:context];
        }
