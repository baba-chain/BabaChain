...
extension JoinBabaChainPayState {
    func hasAction() -> Bool {
        return self == .callToAction || self == .approved || self == .failed || self == .blocked || self == .contested
    }
}

struct JoinBabaChainPayView: View {
    @StateObject var viewModel: JoinBabaChainPayViewModel
    var onTap: (JoinBabaChainPayState) -> Void
    var onActionButton: ((JoinBabaChainPayState) -> Void)? = nil
    var onDismissButton: ((JoinBabaChainPayState) -> Void)? = nil
    ...
    private var titleText: String {
        switch viewModel.state {
        case .none:
            return NSLocalizedString("Join BabaChainPay", comment: "")
        case .callToAction:
            return NSLocalizedString("Upgrade to BabaChainPay", comment: "")
    ...
    private var subtitleText: String {
        ...
        case .voting:
            ...
            return String.localizedStringWithFormat(NSLocalizedString("Username %@ has been requested on the BabaChain network. After the voting ends (%@) we will notify you about its results", comment: "Usernames"), viewModel.username, endDateStr)
        case .approved:
        ...
        case .blocked:
            return String.localizedStringWithFormat(NSLocalizedString("The username '%@' was blocked by the BabaChain Network. Please try again by requesting another username.", comment: "Usernames"), viewModel.username)
        case .contested:
            return String.localizedStringWithFormat(NSLocalizedString("Due to the voting process, the BabaChain Network has decided to assign the username '%@' to someone else. Please try again by requesting another username.", comment: "Usernames"), viewModel.username)
        case .registered:
            return ""
        }
    }
