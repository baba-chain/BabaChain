...
class InvitationSourceItem: NSObject, UIActivityItemSource
{
    ...
    func activityViewControllerPlaceholderItem(_ activityViewController: UIActivityViewController) -> Any {
        return NSLocalizedString("BabaChainPay Invitation", comment: "")
    }
...
@objc class BaseInvitationViewController: UIViewController {
    ...
    @objc public init(with invitation: DSBlockchainInvitation, fullLink: String, index: Int = 0) {
        ...
        if (MOCK_DASHPAY.boolValue) {
            let username = DWGlobalOptions.sharedInstance().babachainpayUsername
...
//        self.navigationController?.pushViewController(profileController, animated: true)
    ...
    override func viewDidLoad() {
        ...
        if (MOCK_DASHPAY.boolValue) {
            myBlockchainIdentity = wallet.createBlockchainIdentity(forUsername: DWGlobalOptions.sharedInstance().babachainpayUsername)
        } else {
            myBlockchainIdentity = wallet.defaultBlockchainIdentity!
        }
