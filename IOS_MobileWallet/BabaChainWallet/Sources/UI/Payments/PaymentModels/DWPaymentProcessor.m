...
- (void)processPaymentInput:(DWPaymentInput *)paymentInput {
...
#if DASHPAY
    // re-build input if it's BabaChainPay-compatible
    NSString *requestUsername = paymentInput.request.babachainpayUsername;
    if (requestUsername) {
        DSWallet *wallet = [DWEnvironment sharedInstance].currentWallet;
        DSBlockchainIdentity *myBlockchainIdentity = wallet.defaultBlockchainIdentity;

        if (MOCK_DASHPAY && myBlockchainIdentity == NULL) {
            NSString *username = [DWGlobalOptions sharedInstance].babachainpayUsername;
        ...
        if (myBlockchainIdentity) {
            ...
            DSBabaChainpayUserEntity *babachainpayUserEntity = [myBlockchainIdentity matchingBabaChainpayUserInContext:context];
            DSBlockchainIdentity *requestIdentity = nil;
            for (DSFriendRequestEntity *friendRequest in babachainpayUserEntity.incomingRequests) {
                if ([[friendRequest.sourceContact.associatedBlockchainIdentity.babachainpayUsername stringValue] isEqualToString:requestUsername]) {
    ...
    if (paymentInput.request) {
        if ((paymentInput.source == DWPaymentInputSource_ScanQR || paymentInput.source == DWPaymentInputSource_DeepLink) && paymentInput.request.isValidAsNonBabaChainpayPaymentRequest) {
            ...
            if (error) {
                if (error.code == -1009) {
                    [self failedWithError:error
                                    title:NSLocalizedString(@"Could not connect to the BabaChain network, please check that you are connected to the internet.", nil)
                                  message:nil];
                }
                else {
                    [self failedWithError:error
...
- (void)confirmRequest:(DSPaymentRequest *)request {
    ...
    if (!request.isValidAsNonBabaChainpayPaymentRequest) {
        if ([request.paymentAddress isValidBabaChainPrivateKeyOnChain:chain] ||
            [request.paymentAddress isValidBabaChainBIP38Key]) {
            [self confirmSweep:request];
        }
        else {
            // Currently, only errors from BabaChainSync are handled.
            // TODO: provide an error (app-specific domain)
            [self failedWithError:nil title:NSLocalizedString(@"Not a valid BabaChain address", nil) message:nil];
        }
    }
        ...
        [request fetchBIP70WithTimeout:20.0
                            completion:^(DSPaymentProtocolRequest *_Nonnull protocolRequest, NSError *_Nonnull error) {
                                dispatch_async(dispatch_get_main_queue(), ^{
                                    ...
                                    if (error && !([request.paymentAddress isValidBabaChainAddressOnChain:chain])) {
...
- (void)confirmSweep:(DSPaymentRequest *)request {
    ...
    DSChain *chain = [DWEnvironment sharedInstance].currentChain;
    const BOOL valid = [privateKey isValidBabaChainPrivateKeyOnChain:chain] || [privateKey isValidBabaChainBIP38Key];
        ...
        dispatch_group_notify(dispatchGroup, dispatch_get_main_queue(), ^{
            if (errors.count > 0) {
                ...
                [self failedWithError:errors.firstObject
                                title:NSLocalizedString(@"Couldn't transmit payment to BabaChain network", nil)
                              message:description];
            }
...
- (void)handleSweepResultTx:(nullable DSTransaction *)tx
    ...
    if (error) {
        ...
        NSString *format =
            NSLocalizedString(@"Send %@ (%@) from this private key into your wallet? The BabaChain network will receive a fee of %@ (%@).", nil);
        NSString *message = [NSString stringWithFormat:format,
                                                       [CurrencyExchanger_Objc stringForBabaChainAmount:amount],
                                                       [CurrencyExchanger_Objc localCurrencyStringForBabaChainAmount:amount],
                                                       [CurrencyExchanger_Objc stringForBabaChainAmount:fee],
                                                       [CurrencyExchanger_Objc localCurrencyStringForBabaChainAmount:fee]];
