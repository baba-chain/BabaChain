...
- (instancetype)initWithAmount:(uint64_t)amount {
    ...
    if (self) {
        _updateQueue = dispatch_queue_create("org.babachain.wallet.DWReceiveModel.queue", DISPATCH_QUEUE_SERIAL);
...
- (void)copyUsernameToPasteboard {
    NSString *username = self.paymentRequest.babachainpayUsername;
...
- (nullable NSString *)requestAmountReceivedInfoIfReceived {
    ...
    uint64_t total = 0;
    const uint64_t fuzz = [CurrencyExchangerObjcWrapper amountForLocalCurrency:[CurrencyExchangerObjcWrapper localCurrencyNumberForBabaChainAmount:1].decimalValue] * 2;
    ...
    for (DSTransaction *tx in wallet.allTransactions) {
        ...
        if (total + fuzz >= request.amount) {
            ...
            NSString *info = [NSString stringWithFormat:NSLocalizedString(@"Received %@ (%@)", nil),
                                                        [CurrencyExchangerObjcWrapper stringForBabaChainAmount:self.amount],
                                                        [CurrencyExchangerObjcWrapper localCurrencyStringForBabaChainAmount:self.amount]];
...
- (void)updateReceivingInfo {
    dispatch_async(self.updateQueue, ^{
        ...
        if (hasAmount) {
            ...
            NSNumber *number = [CurrencyExchangerObjcWrapper localCurrencyNumberForBabaChainAmount:amount];
            if (number) {
...
#if DASHPAY
        paymentRequest.babachainpayUsername = [DWGlobalOptions sharedInstance].babachainpayUsername;
#endif
        ...
        if (paymentRequest && paymentRequest.isValidAsNonBabaChainpayPaymentRequest && rawQRImageData) {
            if (!hasAmount) {
...
#if DASHPAY
            self.username = paymentRequest.babachainpayUsername;
#endif
