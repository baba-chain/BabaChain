...
class SendAmountViewController: BaseAmountViewController {
    ...
    override var actionButtonTitle: String? { NSLocalizedString("Send", comment: "Send BabaChain") }
    ...
    internal func checkLeftoverBalance(isCrowdNodeTransfer: Bool = false, completion: @escaping ((Bool) -> Void)) {
        ...
        if model.amount.plainAmount + CrowdNode.minimumLeftoverBalance > allAvailableFunds {
            let title = NSLocalizedString("Looks like you are emptying your BabaChain Wallet", comment: "Leftover balance warning")
            let message = String
                .localizedStringWithFormat(NSLocalizedString("Please note, you will not be able to withdraw your funds from CowdNode to this wallet until you increase your balance to %@ BabaChain.",
