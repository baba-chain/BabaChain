...
protocol CoinbaseTransactionSendable {
    /// Amount to transfer in BabaChain
...
extension CoinbaseTransactionSendable {
    ...
    func transferFromCoinbase(amount: UInt64, with verificationCode: String?, for idem: UUID? = nil) async throws {
        do {
            _ = try await Coinbase.shared.transferFromCoinbaseToBabaChainWallet(amount: amount, verificationCode: verificationCode, idem: idem)
            await MainActor.run {
