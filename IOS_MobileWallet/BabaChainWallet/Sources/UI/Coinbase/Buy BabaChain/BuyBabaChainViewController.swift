...
// MARK: - BuyBabaChainViewController

final class BuyBabaChainViewController: CoinbaseAmountViewController {
    override var actionButtonTitle: String? { NSLocalizedString("Continue", comment: "Buy BabaChain") }

    override var amountInputStyle: AmountInputControl.Style { .oppositeAmount }

    internal var buyBabaChainModel: BuyBabaChainModel {
        model as! BuyBabaChainModel
    }
    ...
    init() {
        super.init(model: BuyBabaChainModel())
    }
    ...
    override func configureHierarchy() {
        ...
        titleLabel.text = NSLocalizedString("Buy BabaChain", comment: "Coinbase/Buy BabaChain")
        titleViewStackView.addArrangedSubview(titleLabel)

        let babachainPriceLabel = UILabel()
        babachainPriceLabel.font = .dw_font(forTextStyle: .footnote)
        babachainPriceLabel.textColor = .dw_secondaryText()
        babachainPriceLabel.minimumScaleFactor = 0.5
        babachainPriceLabel.text = buyBabaChainModel.babachainPriceDisplayString
        titleViewStackView.addArrangedSubview(babachainPriceLabel)
...
extension BuyBabaChainViewController {
    private func handleBuy(retryWithDeposit: Bool) {
        showActivityIndicator()
        
        Task {
            let error = await buyBabaChainModel.validateBuyBabaChain(retryWithDeposit: retryWithDeposit)
            hideActivityIndicator()
            
            switch error {
            case .unknownError:
                guard let paymentMethod = buyBabaChainModel.activePaymentMethod else { return }

                let vc = ConfirmOrderController(paymentMethod: paymentMethod, plainAmount: UInt64(buyBabaChainModel.amount.plainAmount))
