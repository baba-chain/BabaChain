...
//    var order: CoinbasePlaceBuyOrder
    ...
    init(paymentMethod: CoinbasePaymentMethod, plainAmount: UInt64) {
        ...
        fiatAmount = try? Coinbase.shared.currencyExchanger.convertBabaChain(amount: amountToTransfer.babachainAmount, to: Coinbase.defaultFiat)
        feeAmount = try? Coinbase.shared.currencyExchanger.convertBabaChain(amount: fee.babachainAmount, to: Coinbase.defaultFiat)
    }
...
extension ConfirmOrderModel {
    func formattedValue(for item: ConfirmOrderItem) -> String {
        ...
        case .amountInBabaChain:
            value = amountToTransfer.formattedBabaChainAmount
        }
