...
private var kBabaChainSymbolAssetName = "icon_babachain_currency"
...
extension String {
    ...
    func babachainSymbolAttributedString(with tintColor: UIColor) -> NSAttributedString {
        let image = UIImage(named: kBabaChainSymbolAssetName)!.withTintColor(tintColor)

        let attachment = BabaChainTextAttachment(image: image, verticalOffset: -1)

        return NSAttributedString(attachment: attachment)
    }

    func attributedAmountStringWithBabaChainSymbol(tintColor: UIColor, babachainSymbolColor: UIColor? = nil, font: UIFont? = nil) -> NSAttributedString {
        let attributedString = NSMutableAttributedString(string: trimmingCharacters(in: .whitespacesAndNewlines))

        let babachainSymbolAttributedString = babachainSymbolAttributedString(with: babachainSymbolColor ?? tintColor)
        let babachainSymbolRange = attributedString.string.nsRange(of: DASH)
        if babachainSymbolRange.isValid {
            attributedString.replaceCharacters(in: babachainSymbolRange, with: babachainSymbolAttributedString)
        } else {
            attributedString.insert(.init(string: " "), at: 0)
            attributedString.insert(babachainSymbolAttributedString, at: 0)
        }

        let amountLocation = babachainSymbolRange.lowerBound == 0 ? 2 : 0
    ...
    /// Convert string to plain babachain amount
    ///
    /// - Parameters:
    ///   - locale: Locale to use when converting string to decimal
    ///
    /// - Returns: Plain babachain amount or nil
    ///
    /// - Note: This method expects string to be a number otherwise it returns nil
    func plainBabaChainAmount(locale: Locale? = nil) -> UInt64? {
        guard let babachainNumber = Decimal(string: self, locale: locale) else { return nil }

        let duffsNumber = Decimal(DUFFS)
        let plainAmount = babachainNumber * duffsNumber
        let babachainAmount = NSDecimalNumber(decimal: plainAmount)

        return babachainAmount.uint64Value
    }
