...
+ (BOOL)canHandleURL:(NSURL *)url {
    ...
    return [url.scheme isEqual:@"babachain"] || [url.scheme isEqual:@"babachainwallet"] || [url.scheme isEqual:@"pay"];
}
...
+ (nullable DWURLAction *)actionForURL:(NSURL *)url {
    ...
    if ([url.scheme isEqual:@"babachainwallet"]) {
    ...
    else if ([url.scheme isEqual:@"babachain"] || [url.scheme isEqual:@"pay"]) {
        DWURLPayAction *action = [[DWURLPayAction alloc] init];
...
+ (nullable NSURL *)paymentURLWithParams:(NSDictionary<NSString *, NSString *> *)params {
    ...
    NSString *componentsString = [NSString stringWithFormat:@"babachain:%@", mutableParams[@"pay"]];
