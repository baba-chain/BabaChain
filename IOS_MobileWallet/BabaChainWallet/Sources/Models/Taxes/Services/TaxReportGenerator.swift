...
enum TaxReportGenerator {
    ...
    static func generateCSVReport(completionHandler: @escaping (_ fileName: String, _ file: URL) -> Void,
        ...
        guard DWEnvironment.sharedInstance().currentChainManager.syncPhase == .synced else {
            let error = NSError(domain: "BabaChainWallet",
                                code: 500,
        ...
        DispatchQueue.global(qos: .default).async {
            ...
            guard let documentsDir = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first else {
                errorHandler(NSError(domain: "BabaChainWallet",
                                     code: 500,
    ...
    private static func value(for column: ReportColumns, transaction: DSTransaction, andUserInfo userInfo: TransactionMetadata?) -> String {
        ...
        case .sentQuantity:
            ...
            guard transaction.babachainAmount != UInt64.max else { return "" }
            let babachainAmount = transaction.babachainAmount + fee
            let formattedNumber = NumberFormatter.csvBabaChainFormatter.string(from: babachainAmount.babachainAmount as NSDecimalNumber) ?? ""
