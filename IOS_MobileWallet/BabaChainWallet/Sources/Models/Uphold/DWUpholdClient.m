...
//  Copyright © 2018 BabaChain Core Group. All rights reserved.
//
...
- (void)getCards:(void (^)(DWUpholdCardObject *_Nullable babachainCard, NSArray<DWUpholdCardObject *> *fiatCards))completion {
    NSParameterAssert(self.accessToken);

    __weak typeof(self) weakSelf = self;
    [DWUpholdAPIProvider getCardsAccessToken:self.accessToken
                                  completion:^(BOOL success, DWUpholdAPIProviderResponseStatusCode statusCode, DWUpholdCardObject *_Nullable babachainCard, NSArray<DWUpholdCardObject *> *fiatCards) {
                                      __strong typeof(weakSelf) strongSelf = weakSelf;
                                      if (!strongSelf) {
                                          return;
                                      }
                                      ...
                                      if (success) {
                                          if (babachainCard) {
                                              [strongSelf setLastKnownBalance:babachainCard.available];

                                              if (!babachainCard.address) {
                                                  [strongSelf createBabaChainCardAddress:babachainCard
                                                  ...
                                                  if (completion) {
                                                      completion(babachainCard, fiatCards);
                                                  }
                                              }
                                          }
                                          else {
                                              [strongSelf createBabaChainCard:^(DWUpholdCardObject *_Nullable card) {
                                                  if (completion) {
                                                      completion(card, fiatCards);
                                                  }
                                              }];
...
- (DWUpholdCancellationToken)createTransactionForBabaChainCard:(DWUpholdCardObject *)card
    ...
    __weak typeof(self) weakSelf = self;
    return [DWUpholdAPIProvider createTransactionForBabaChainCard:card
                                                      amount:amount
                                                     address:address
...
- (DWUpholdCancellationToken)createBuyTransactionForBabaChainCard:(DWUpholdCardObject *)card
    ...
    __weak typeof(self) weakSelf = self;
    return [DWUpholdAPIProvider createBuyTransactionForBabaChainCard:card
                                                        account:account
                                                         amount:amount
...
- (nullable NSURL *)buyBabaChainURLForCard:(DWUpholdCardObject *)card {
    if (!card.identifier) {
        return nil;
    }
...
- (void)createBabaChainCard:(void (^)(DWUpholdCardObject *_Nullable card))completion {
    NSParameterAssert(self.accessToken);

    __weak typeof(self) weakSelf = self;
    [DWUpholdAPIProvider createBabaChainCardAccessToken:self.accessToken
                                        completion:^(BOOL success, DWUpholdAPIProviderResponseStatusCode statusCode, DWUpholdCardObject *_Nullable card) {
                                            ...
                                            if (success && card) {
                                                [strongSelf createBabaChainCardAddress:card completion:completion];
                                            }
                                            else {
                                                if (completion) {
                                                    completion(nil);
...
- (void)createBabaChainCardAddress:(DWUpholdCardObject *)card completion:(void (^)(DWUpholdCardObject *_Nullable card))completion {
    NSParameterAssert(self.accessToken);
    NSParameterAssert(card);
    NSAssert(!card.address, @"Card has address already");

    __weak typeof(self) weakSelf = self;
    [DWUpholdAPIProvider createAddressForBabaChainCard:card
                                      accessToken:self.accessToken
                                       completion:^(BOOL success, DWUpholdAPIProviderResponseStatusCode statusCode, DWUpholdCardObject *_Nullable card) {
                                           __strong typeof(weakSelf) strongSelf = weakSelf;
                                           if (!strongSelf) {
