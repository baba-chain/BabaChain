...
final class CoinJoinMixingTxSet: GroupedTransactions, TransactionWrapper {
    ...
    var infoText: String {
        NSLocalizedString("Your BabaChain was mixed using these transactions.", comment: "CoinJoin") // MO-720 BEF
    }
    
    var fiatAmount: String {
        (try? CurrencyExchanger.shared.convertBabaChain(amount: (UInt64(abs(amount))).babachainAmount, to: App.fiatCurrency).formattedFiatAmount) ??
