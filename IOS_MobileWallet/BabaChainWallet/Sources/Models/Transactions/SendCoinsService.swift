...
public final class SendCoinsService: NSObject {
    ...
    func payWithBabaChainUrl(url paymentUrlString: String) async throws -> DSTransaction {
        // Create payment input from the URL
        guard let paymentUrl = URL(string: paymentUrlString) else {
            throw BabaChainSpendError.paymentProcessingError("Invalid payment URL")
        }
        ...
        guard paymentInput.request?.r != nil else {
            throw BabaChainSpendError.paymentProcessingError("Invalid payment request")
        }
    ...
    private func completePayment(transaction: DSTransaction?, error: Swift.Error?) {
        ...
        if let error = error {
            ...
            continuation.resume(throwing: BabaChainSpendError.paymentProcessingError("No transaction returned"))
        }
    }
}
...
extension SendCoinsService: DWPaymentProcessorDelegate {
    ...
    public func paymentProcessor(_ processor: DWPaymentProcessor, requestAmountWithDestination sendingDestination: String, details: DSPaymentProtocolDetails?, contactItem: DWDPBasicUserItem?) {
        completePayment(transaction: nil, error: BabaChainSpendError.paymentProcessingError("Request is missing destination"))
    }
    ...
    public func paymentProcessorDidCancelTransactionSigning(_ processor: DWPaymentProcessor) {
        completePayment(transaction: nil, error: NSError(domain: "BabaChainSpend", code: -2, userInfo: [NSLocalizedDescriptionKey: NSLocalizedString("Transaction cancelled", comment: "")]))
    }
