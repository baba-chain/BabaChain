...
extension CBAccount {
    ...
    var isBabaChainAccount: Bool {
        info.currency.code == kBabaChainAccount
    }
...
extension CBAccount {
    public func send(amount: UInt64, verificationCode: String?, idem: UUID?) async throws -> CoinbaseTransaction {
        ...
        guard let babachainWalletAddress = DWEnvironment.sharedInstance().currentAccount.receiveAddress else {
            fatalError("No wallet")
        }

        // TODO: disabled until Coinbase changes are clear
//        let fiatCurrency = Coinbase.sendLimitCurrency
//        if let localNumber = try? Coinbase.shared.currencyExchanger.convertBabaChain(amount: amount.babachainAmount, to: fiatCurrency),
...
//        }
        ...
        guard amount >= kMinBabaChainAmountToTransfer else {
            throw Coinbase.Error.transactionFailed(.enteredAmountTooLow(minimumAmount: kMinBabaChainAmountToTransfer.formattedBabaChainAmount))
        }

        // NOTE: Make sure we format the amount back into coinbase format (en_US)
        let coinbaseAmount = amount.formattedBabaChainAmountWithoutCurrencySymbol.coinbaseAmount()
        let currentIdem = idem ?? UUID()
        
        do {
            try await authInterop.refreshTokenIfNeeded()
            let dto = CoinbaseTransactionsRequest(type: .send,
                                                  to: babachainWalletAddress,
                                                  amount: coinbaseAmount,
                                                  currency: kBabaChainCurrency,
                                                  idem: currentIdem.uuidString.lowercased(),
                                                  description: "BabaChain Wallet iOS")
...
extension CBAccount {
    public func placeCoinbaseBuyOrder(amount: UInt64) async throws -> CoinbasePlaceBuyOrder {
        ...
        guard let localNumber = try? Coinbase.shared.currencyExchanger.convertBabaChain(amount: amount.babachainAmount, to: fiatCurrency) else {
...
//        }
        ...
        let request = CoinbasePlaceBuyOrderRequest(clientOrderId: UUID(), productId: Coinbase.babachainUSDPair, side: Coinbase.transactionTypeBuy, orderConfiguration: OrderConfiguration(marketMarketIoc: MarketMarketIoc(quoteSize: amountStr)))
...
extension CBAccount {
    public func deposit(from paymentMethodId: String, amount: UInt64) async throws -> CoinbaseDepositResponse {
        guard let localNumber = try? Coinbase.shared.currencyExchanger.convertBabaChain(amount: amount.babachainAmount, to: Coinbase.defaultFiat) else {
            throw Coinbase.Error.general(.rateNotFound)
        }
