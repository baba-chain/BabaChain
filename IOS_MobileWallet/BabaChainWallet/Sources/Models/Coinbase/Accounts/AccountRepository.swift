...
class AccountRepository {
    ...
    var babachainAccount: CBAccount? {
        cachedAccounts[kBabaChainAccount]
    }
    
    var usdAccount: CBAccount? {
        cachedAccounts[Coinbase.babachainUSDPair]
    }

    init(authInterop: CBAuthInterop) {
        self.authInterop = authInterop

        if let babachainUser = accountManager.storedAccount(with: authInterop) {
            cachedAccounts[kBabaChainAccount] = babachainUser
        }
    }
