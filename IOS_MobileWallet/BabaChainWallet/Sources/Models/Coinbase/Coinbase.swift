...
let kMaxBabaChainAmountToTransfer: UInt64 = kOneBabaChain
let kMinUSDAmountOrder: Decimal = 1.99
let kMinBabaChainAmountToTransfer: UInt64 = 10_000
...
class Coinbase {
    ...
    private func prefetchData() {
        Task {
            try await accountService.refreshAccount(kBabaChainAccount)
...
extension Coinbase {
    ...
    var babachainAccount: CBAccount? {
        accountService.babachainAccount
    }
...
extension Coinbase {
    ...
    public func createNewCoinbaseBabaChainAddress() async throws -> String {
        do {
            let address = try await accountService.retrieveAddress(for: kBabaChainAccount)
    ...
    public func getBabaChainExchangeRate() async throws -> CoinbaseExchangeRate? {
        do {
            return try await coinbaseService.getCoinbaseExchangeRates(currency: kBabaChainCurrency)
        } catch Coinbase.Error.userSessionRevoked {
            try await auth.signOut()
            throw Coinbase.Error.userSessionRevoked
        } catch {
    ...
    public func transferFromCoinbaseToBabaChainWallet(amount: UInt64,
                                                 verificationCode: String?,
                                                 idem: UUID?) async throws -> CoinbaseTransaction {
        do {
            let tx = try await accountService.send(from: kBabaChainAccount, amount: amount, verificationCode: verificationCode, idem: idem)
    ...
    ///
    /// - Parameters:
    ///   - amount: Plain amount in BabaChain
    ///
    ...
    func placeCoinbaseBuyOrder(amount: UInt64) async throws -> CoinbasePlaceBuyOrder {
        do {
            return try await accountService.placeBuyOrder(for: kBabaChainAccount, amount: amount)
        } catch Coinbase.Error.userSessionRevoked {
            try await auth.signOut()
            throw Coinbase.Error.userSessionRevoked
        }
