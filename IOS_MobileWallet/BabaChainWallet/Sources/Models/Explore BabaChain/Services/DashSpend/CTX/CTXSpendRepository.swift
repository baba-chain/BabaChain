...
class CTXSpendRepository: CTXSpendTokenProvider, BabaChainSpendRepository {
                        ...
                        if let fiatAmountErrors = errorData.fields?.fiatAmount,
                           let firstFiatError = fiatAmountErrors.first {
                            if firstFiatError == "above threshold" || firstFiatError == "below threshold" {
                                throw BabaChainSpendError.purchaseLimitExceeded
                            }
                        }
    ...
    private func mapError(_ error: Error) -> Error {
        if let ctxError = error as? BabaChainSpendError {
            return ctxError
        }
        
        return BabaChainSpendError.networkError
    }
}
