...
public class ExploreDatabaseSyncManager {
    ...
    init() {
        ...
        let isMainnet = DWEnvironment.sharedInstance().currentChain.isMainnet()
        if isMainnet {
            databasePath = "gs://babachain-wallet-firebase.appspot.com/explore/explore-v4.db"
        } else {
            databasePath = "gs://babachain-wallet-firebase.appspot.com/explore/explore-v4-testnet.db"
        }
            ...
            if let e = error {
                ...
                Task {
                    do {
                        ...
                        DSLogger.log("ExploreBabaChain: failed to open DB archive: \(String(describing: error))")
